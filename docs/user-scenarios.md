...
> - **S-CONFIG-SOT-2 되쓰기 순서(레이스 차단)**: `syncConfigToFile()` 은 파일→localStorage 하이드레이션 **완료 후에만** 실행(hydrated 플래그 게이트). 하이드레이션 전 스테일 되쓰기 금지. stale-URL 대비 sync 는 하이드레이트 **후** 재실행으로 충족.
...
| **S-CASCADE-T3** (원격 cascade, 2026-07-08) | 고급 사용자가 **원격/직접운영 cascade 파사드 URL**(다른 PC·WSL 안 cascade 등)을 설정해 연결 — 접힌 고급 섹션 격리·naiaKey 게이트·http/https 검증·egress 경고. 로컬 auto-spawn(T1) 켜져 있으면 로컬 우선(원격 무시). WSL cascade도 이 URL 경로로 붙음(셸이 WSL 부트스트랩 안 함 — in-shell WSL 셋업은 죽은 레거시) | 제어면(설정) — cascade 소스 T3(계약=파사드 URL, 프로세스 무관) | `config.test.ts`(normalizeCascadeUrl 4)·`capability-settings.spec.ts`(FR-8 URL 검증·게이트 3). ⚠️ 실 원격 파사드 도달=실 백엔드 |
...
> - **UC-AV.6 T3 원격 cascade URL(고급)**: `cascadeRuntimeUrl`(http/https만, `normalizeCascadeUrl` 검증·정규화) 로 원격/직접운영 파사드 연결 — 접힌 고급 섹션·naiaKey 게이트·egress 경고. 로컬 T1 우선(localFacadeUrl 있으면 원격 무시). WSL 안 cascade 도 이 경로(셸이 WSL 부트스트랩 안 함 — in-shell WSL 셋업은 죽은 레거시). 검증: `config.test.ts`(normalizeCascadeUrl 4)·`capability-settings.spec.ts` FR-8(URL 검증·게이트).

...
| **S-SKILLS-CLI** (#605/#606) | 사용자가 설정 → 스킬 탭을 열면 설치된 코딩 CLI(Claude·Codex·Grok·Antigravity CLI)만 체크박스로 보이고, 유튜브 등 셸 몸짓 토글이 둘째 묶음으로 보인다. 로그인·다시 확인 버튼으로 준비 상태를 고친다. 켠 CLI 이름은 채팅 턴에 에이전트로 전달된다. | FR-SKILLS-CLI.1–3 | `SkillsTab.test.tsx` · `cli-detection.test.ts` · `e2e/skills-cli.spec.ts` · e2e-tauri 14/19/28/59 · 96-readiness |

**S-SKILLS-CLI 상태 매트릭스 (P02):** 기본/성공은 `SkillsTab.test.tsx`와 `e2e/skills-cli.spec.ts`의 설치된 CLI 카드·준비 상태·접근 가능한 체크박스로, 빈 목록은 `skills-cli-empty`, 진행은 새로 고침 비활성화/감지 중, 오류는 `role=alert`와 stale 카드 제거·새로 고침 복구, 좁은 폭은 480px viewport overflow 검증으로 매핑한다. Native Tauri `e2e-tauri/specs/14-skills-tab.spec.ts`, `19-skills-bulk.spec.ts`, `28-skills-install.spec.ts`, `59-skills-interactions.spec.ts`는 동일 DOM 계약과 실제 Rust IPC 경계를 확인한다.
...
| S04 | naia 계정 / api key 설정 | UC12 | control-plane(entitlement·naia-token) | 측정 — FR-V017.2: ADK에 남은 이전 direct-provider apiKey를 `naia-anyllm`에 재사용하지 않고 선택된 Naia credential/reference만 전달 |

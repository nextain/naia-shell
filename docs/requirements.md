...
> **2026-07-22 bounded playback boundary:** `skill_youtube_bgm` now returns a
> request receipt rather than a current-playing claim. The selected title is
> withheld from Shell-to-agent current-track context until the active local
> iframe reports `playing`; stale playback ids and lower sequences are ignored.
> This is only the FR-RADIO-DJ.1/2 fact boundary, not completion of the
> separate agent-owned `skill_radio_dj`, proactive speech permit, or weather
> requirements below.

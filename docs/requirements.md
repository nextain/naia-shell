...
| ID | Normative requirement | Verification |
|---|---|---|
| **FR-LLM-TOOL-CALL-MEASUREMENT.1** | The #592 measurement is based on Shell commit `2941e3b4ac644cffb5be93d72991bb8e9e81f0d0`, Shell package version `0.2.3`, and the paired Agent commit `1c2561db486c24c31d10ddbef5ca5f0ff766c7ad`. The measurement branch may contain only its declared harness/docs changes; a dirty or mismatched checkout is rejected before a live request. The exact paired Agent is built with `pnpm --dir <agentRoot> build`, its source worktree is checked again, and provenance includes the compiled `dist/main`, Agent proto, and Shell catalog source digests. | `scripts/measure-agent-tool-calling.mjs` provenance, clean-pair preflight, and build/digest gate |
| **FR-LLM-TOOL-CALL-MEASUREMENT.2** | The measurement sends the five #592 request families to `deepseek-v4-flash` and `gpt-5.6-luna`, ten repetitions per model and family. It registers Shell's full always-on app catalog: `skill_youtube_bgm`, `skill_environment`, and all 12 `env_browser_*` descriptors. Registration is verified through Agent `listSkills`. Each attempt records accepted tool names, schema-valid arguments, a correlated successful tool result, a final response after that result, latency, and a failure classification. | `scripts/measure-agent-tool-calling.test.mjs` fixture cases and aggregation; `docs/regression-runs/naia3090-*-tool-calling.json` live receipt |
| **FR-LLM-TOOL-CALL-MEASUREMENT.3** | A result meets the per-model/per-family gate only when at least 9 of 10 attempts satisfy all tool-call, argument, correlated successful-result, follow-up, and completion conditions with no error event. Setup failures, gateway/provider failures, tool execution failures, and Agent follow-up failures remain distinct. The complete report is redacted for credential-shaped fields and JSON strings, the `NAIA_API_KEY` value is never written to a tracked path, and live output is atomically written under `docs/regression-runs/`. | measurement threshold, failure-classification, catalog, redaction, output, and report-shape tests |

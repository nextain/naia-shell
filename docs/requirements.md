...
| **FR-VOICE.11** | 6GB VoxCPM2 음성 경로는 스트리밍 답변을 문장 단위로 나누되 합성은 단일-flight로 직렬화하고 재생은 독립 오디오 큐로 이어간다. 첫 WAV의 생성 시간/재생 길이로 RTF를 측정해 RTF>1이면 다음 문장까지 선행 버퍼링하고, RTF≤1 또는 한 문장 답변이면 즉시 재생한다. 중단·실패는 대기 합성을 폐기하고 다음 문장의 순서를 막지 않는다. (#688 개정: "다음 문장까지 선행 버퍼링" 대신 첫 문장 1초 여유 후 재생 — FR-VOICE.19 참고) | UC-WIN-VOICE-6G | `ChatArea.test.tsx` 6GB 직렬 합성·적응형 버퍼·중단 + `audio-queue.test.ts` pause/resume·WAV duration + 실제 8901 429 무발생 연속 발화 |
...
| **FR-VOICE.19** | **#688 로 개정 (2026-09-22, 루크 결정 원문: 「음성은 문장 준비되는대로 바로 들려주게 해. … 생성후 1초정도만 여유주고 플레이 하는건 어떨까 싶네.」)** — 워밍업 홀드(complete-then-play)는 폐기. 턴의 첫 오디오가 준비되면 재생을 연다: 합성이 실시간보다 느리면(RTF>1) 1초 여유 뒤, 아니면 즉시. 이후 문장은 준비되는 대로 이어 재생하고 다시 붙잡지 않는다. 실패는 즉시 해제, interrupt 는 대기 중인 여유 타이머를 취소한다. 아래 원래 조항 ②③ 은 이력으로만 남긴다. **워밍업 언더런 근본 차단** (#519). ① RTF 측정은 첫 문장 한정이 아니라 모든 로컬 문장에서 수행한다. ② RTF>1 관측 시 warming hold: 재생 시작을 보류하고 `naia:voice-model-preparing` 이벤트로 "음성 모델 준비 중…"을 표시한다. 폴백 없음(FR-VOICE.2 유지 — 루크 결정 2026-08-31). ③ 해제는 (a) 어떤 문장이 RTF<1로 완성(엔진 웜), (b) 스트림 종료 + 대기 문장 전부 합성 완료(complete-then-play — 갭 원리적 0), (c) 문장 실패(교착 방지) 세 조건뿐이며 임의 시간 캡으로 굶주린 큐를 강제 재생하지 않는다(기존 5초 캡 대체). ④ interrupt 는 hold 와 preparing 표시를 함께 해제한다. | UC-WIN-VOICE-WARMUP | `local-voice-scheduler.test.ts` 1s 여유·즉시재생·후속문장 무홀드·실패해제·interrupt 타이머 취소 + `sentence-pipeline.test.ts` 전 문장 측정 |
| **FR-VOICE.20** | **로컬 음성 스트리밍 재생 계약 (2026-09-11)**. ① 셸은 로컬 음성 호스트에 `response_format:"pcm16"`, `stream:true` 로 요청하고, 호스트가 `audio/pcm;rate=<hz>` 로 답하면 응답 본문을 청크로 읽어 도착하는 대로 재생한다 — 한 문장의 합성이 끝나기를 기다리지 않는다. 홀수 바이트로 끊긴 경계는 다음 청크로 이어 붙여 해석하고, 전체 청크는 기존 소비자(길이·RTF·진단)를 위해 같은 RIFF/WAVE base64 로 조립해 돌려준다. ② 재생 큐는 문장 순서를 유지한다 — 뒤 문장의 스트림이 먼저 도착해도 앞 문장 다음에 재생하고, 첫 청크에서 재생 시작을 알리며, 청크는 앞 청크가 끝나는 시각에 이어 붙여 예약한다. 중단(`clear`)은 예약된 소스를 모두 멈추고 이후 도착하는 청크를 버린다. ③ 슬라이드 낭독문은 채팅 응답과 같이 문장 단위로 나뉘어 이 파이프라인에 들어간다(조각 단위 `VITE_NAIA_SLIDES_TTS_CHUNK` = word|phrase|sentence, 기본 sentence). ④ 스트리밍을 모르는 호스트는 지금까지처럼 WAV 한 벌로 답하고, 셸은 예약해 둔 스트림 슬롯을 풀어 기존 WAV 재생 경로로 되돌린다 — 낭독이 사라지지 않는다. (#688 정정: 스트림 슬롯은 이미 재생 큐로 넘어가 있으므로, WAV 는 같은 슬롯에서 재생된다 — seq 재삽입은 불가) ⑤ 낭독이 아바타를 idle↔talking 으로 계속 왕복시키므로, 아바타 클립 재생은 한 번 로드한 클립에 `src` 를 다시 대입하지 않는다 — WebKitGTK 에서 `src` 대입이 이전 미디어 파이프라인을 메인 스레드에서 동기 해체하다 그 파이프라인의 demuxer 스레드와 서로 기다려 웹뷰 전체가 멈춘다(2026-09-11 gdb 재현: prebaked 렌더러 3/3 · layered 플레이어 2/2). 실사용 경로인 prebaked 렌더러는 클립 URL 마다 `<video>` 를 하나씩 두고(마운트 요소 + 같은 부모의 숨은 형제) 전환 때 재생 요소만 바꾸며, 정지하면 자기가 만든 형제 요소를 DOM 에서 거둔다. layered 플레이어의 더블버퍼도 back 이 이미 같은 클립을 들고 있으면 되감아 교체한다. 로드가 취소된 클립은 캐시로 취급하지 않는다. | S-SLIDES-NARRATION-LATENCY (UC9 앱) | `synthesize.test.ts` 스트리밍 요청·청크 해석·WAV 조립·WAV 폴백 + `audio-queue.test.ts` 순서·첫 청크 시작·이어 붙임·정지 + `sentence-pipeline.test.ts` 슬롯 선예약·미지원 큐·청크 0개 폴백·실패 슬롯 해제·첫 청크 재생 해제 + `local-voice-scheduler.test.ts` 첫 청크 워밍업 해제·느린 첫 청크 유지·세대 불일치 + `ChatArea.test.tsx` 낭독문 문장 분할 + `prebaked-renderer.test.ts` 클립당 src 1회·활성 요소만 play·형제 요소 삽입과 정지 시 회수 + `nva-layered-player.test.ts` 왕복 무재로드·되감기·취소 후 재로드 |
| **FR-VOICE.21** | **호스트 음성 런타임 종료를 숨기지 않는다** (#672). ① `spawn_voxcpm2` 가 stdout 단절·자식 종료를 보면 `{status:?}` 의 `(None)` 만 보여 주지 않는다. 종료 코드·시그널, 또는 "아직 살아 있거나 종료 상태를 읽지 못함"을 적고 `voxcpm2-stderr.log` 끝줄을 사용자 오류에 붙인다. WDAC / os error 4551 / `voxcpm2_tensorrt.activation` DLL 실패가 있으면 그 뜻을 한 줄로 말한다. ② 설치·기동이 실패하면 `voxcpm2_install_progress {phase:"failed"}` 를 보내고 Settings는 마지막 진행률(Windows 에서 자주 40%)을 지운다. 런타임이 이미 죽었는데 설치 바가 남아 있지 않는다. ③ GPU 를 고른 뒤 부모 환경의 `VOXCPM_DEVICE=cpu` 를 자식에게 물려 주어 AudioVAE 를 CPU 로 내리지 않는다. 고른 카드가 있으면 `VOXCPM_DEVICE=cuda` 다. GPU 라이브러리가 WDAC 로 막히면 CPU 로 조용히 가지 않고 ①의 오류로 말한다. | UC-V017-VOXCPM2-RUNTIME-EXIT-DIAGNOSTICS | Rust `format_voxcpm2_runtime_exit`·`resolve_torch_device` 단위 + `SettingsTab.test.tsx` 40% 해제·failed phase + `platform-matrix.test.ts` Windows 설치 진행 40% 이후 |
| **FR-VOICE.22** | **느린 로컬 음성 호스트 전체 WAV 재생 및 상태 해제 정직화** (#688). ① 스트리밍을 지원하지 않는 로컬 호스트가 전체 WAV 로 응답할 때, 셸은 이미 예약된 순서 슬롯에서 해당 WAV 를 직접 재생한다(`endWithAudio`) — 슬롯이 큐로 이동한 뒤 seq 로 재삽입하지 않아 WAV 가 고아로 남지 않는다. ② 파이프라인 진행 상태(`tts`/`render`)는 아직 발화되지 않은 문장이 남아 있는 동안에만 수락되고, 문장이 이미 공개된 뒤 시작된 늦은 직렬 합성은 상태 칩을 다시 띄우지 않아 "음성 처리 중…" 이 잔존하지 않는다. ③ 로컬 합성 요청마다 90초의 per-attempt 타임아웃을 적용해 초과 시 `LocalVoiceTimeoutError` 로 실패 처리하며 엔진 시작으로 재시도하지 않는다(호출자 취소는 기존 취소 상태 유지). ④ `naia:voice-model-preparing` 의 `detail: false` 이벤트는 해당 호출이 실제로 `detail: true` 를 알렸을 때에만 발생시킨다. ⑤ busy(429) 예산 소진이나 서버 5xx 는 로컬 음성 1회 실패 알림으로 표면화한다. ⑥ 재생 창 정책은 FR-VOICE.19 개정(#688, 첫 오디오 + 1초 여유, 이후 문장은 준비되는 대로). 상태: Done (P05, 2026-09-22 — vitest 2028 통과, 실기 E2E 는 루크 대기) | UC-WIN-VOICE-SLOW-HOST | `audio-queue.test.ts` whole-audio 슬롯 재생·일시정지 후 재개 + `sentence-pipeline.test.ts` real AudioQueue 재생·늦은 직렬 작업 상태 방지 + `reveal-guard.test.ts` acceptPipelineOutputStage 조건 + `synthesize.test.ts` 90초 타임아웃·호출자 취소·429 소진·정직한 preparing 라벨 |
| **FR-VOICE.23** | **아바타 반복 영상의 WebKitGTK 정지 방지 (2026-09-25)**. WebKitGTK 에서 영상 싱크의 스트리밍 스레드는 싱크 잠금을 쥔 채 `MediaPlayerPrivateGStreamer::triggerRepaint` 안에서 메인 스레드가 프레임을 그려 주기를 기다린다. 이때 메인 스레드가 같은 잠금이 필요한 일을 하면 서로 기다리며 웹뷰 전체가 멈춘다. 재생 중인 요소에 대해 이런 일을 하는 경로는 세 가지다. ① `loop` 속성이 끝에서 되감는 경로(didEnd → mediaPlayerTimeChanged → seekInternal → doSeek; 2026-09-24 녹화 셸 gdb, 격리 부하 시험 재현). ② JS 의 `currentTime` 대입(seekTask → doSeek). ③ JS 의 `pause()`(pauseInternal → set_state(PAUSED)). 그래서 실사용 렌더러(prebaked)는 반복 클립을 `loop` 없이 두 `<video>` 가 번갈아 끝까지 재생하게 한다(`TwinLoop`). 끝나면 처음에 멈춰 첫 프레임을 준비해 둔 짝 요소가 이어받고, 끝난 요소는 스트림 끝에서 브라우저가 스스로 멈춘 뒤 250ms 가 지나 숨은 상태에서만 되감는다. 다른 클립으로 전환할 때 반복 클립은 멈추지 않고 이번 회차를 숨은 채 끝낸 뒤 이어받지 않는다. 단, 말하는 동안에는 idle 반복을 떠나지 않고 talking 반복 아래에서 계속 돌린다(FR-NVA-WEB.5 문턱이 쉼마다 바로 닫힌 입으로 돌아가게). 이것도 play 만 하므로 같은 규칙 안이다. 한 번 재생 클립은 음소거해 끝까지 흘려보낸다. 이미 돌고 있는 반복을 다시 요청하면 되감지 않고 그대로 이어서 보여 준다. 파이프라인이 스스로 멈춰 재생 중이라면서 시간이 3초 넘게 그대로인 요소는 끝난 것처럼 짝에게 넘기되 건드리지 않고 음소거해 두며, 새 요소로 자리를 채우고, 그 요소가 스스로 끝날 때만 거둔다(수정 전후 모두 부하 시험에서 관찰된 멈춤). 렌더러 정지(언마운트) 때의 `pause()` 는 남는다. | S-SLIDES-NARRATION-LATENCY (UC9 앱) | `twin-loop.test.ts` 교대·지연 되감기·재생 중 무되감기·stop 무정지·멈춘 요소 교체 + `prebaked-renderer.test.ts` loop 속성 0·재생 중 요소의 seek/pause 0·떠난 반복의 자연 종료 + 격리 WebKitGTK 2.52.5 부하 시험(Xvfb·격리 PipeWire, 메인 스레드를 20~80ms 씩 붙잡는 조건, 20초 무응답=정지): 수정 전 렌더러 15회 중 8회 정지(`loop` 경로 5·`currentTime` 1·`pause()` 2, 36~722초), 최종 렌더러 8회 × 11분 정지 0, 정지 감시를 넣기 전 단계 8회 × 11분도 정지 0(짧은 클립 반복 교대 수천 회, 녹화 셸 립싱크 변형 포함) |
| **FR-SHELL-ISO.1** | **개발 인스턴스 격리 + 단일 GPU 런타임 공유** (#425). ① 개발 실행(tauri:dev/tauri:prod)은 별도 앱 정체성(`Naia Dev`/`com.naia.shell.dev`, tauri.conf.dev.json 오버레이 — WebView2 데이터·localStorage 분리)과 별도 데이터 홈(`NAIA_HOME`, 기본 `~/.naia-dev` — adk-path 캐시·lease·logs·run·skills)을 쓴다. 설치본(stage-runtime)은 base config·`~/.naia` 유지. native E2E 격리(e2e_runtime_dir)가 항상 상위. ② cascade(VoxCPM2, :8910)는 **두 인스턴스가 공유**: 부팅·시작 시 `/health`가 TTS enabled 면 죽이지 않고 **입양**(ADOPTED_CASCADE_READY — 스폰과 동일 계약), 바인드됐지만 비건강일 때만 고아 정리. 정지는 공유 엔진 정지. ③ 포트 분리(2026-08-06 dual-instance 설계 수확): dev 인스턴스는 BGM 사이드카 :18891, OAuth 콜백 :18892 를 쓴다(`dev-instance.mjs` env + Rust `development_instance_enabled()` — debug 빌드 AND NAIA_DEV_INSTANCE=1 이중 게이트, e2e 센티널 무접촉). dev 인스턴스는 updater 도 비활성. 게이트웨이 :18789 는 spawn_gateway 가 제거된 스텁(레거시)이라 분리 불요 판정. ④ agent 기억 격리 = naia-agent#108 완료(NAIA_HOME 존중, 페어 94a7b1d). ⑤ (#703) 로컬 음성 런타임의 **고정 패키지 바이트**(내려받은 zip, payload, NVIDIA 패키지, 모델 리비전, 기본 음성 팔레트, GPU별 엔진 세대)는 데이터 홈 밖의 사용자별 캐시(`%LOCALAPPDATA%\NaiaRuntimeCache`, Linux `$XDG_DATA_HOME/naia-runtime-cache`)에 내용 주소로 한 벌만 둔다. 사용자 음성·`state/`·로그·PID는 절대 캐시에 두지 않고 모드별 데이터 홈에 남긴다. native E2E 격리가 여전히 상위다(E2E 기본은 개인 캐시, 사용자 캐시는 명시적 읽기 전용 opt-in). 한 모드의 제거는 캐시를 지우지 않는다. 캐시 해석기는 `DataHomeChild` 가 아닌 별도 모듈(`voice_cache.rs`)이다. 디스크의 슬롯·엔진 디렉터리 이름은 64자 키의 앞 16자다(관리자 권한 없이는 Windows 경로 260자 제한을 넘을 수 없어서, #705 실기에서 pip 가 실패). 전체 64자 키는 slot.json·stamp.json·active.json 에 두고 준비 판정은 전체 키로 비교하므로, 앞 16자가 같아도 불일치로 처리된다. 이전 설치에서 옮겨 온 슬롯은 아티팩트 매니페스트·네이티브 해시·준비 표식·엔진 매니페스트를 확인한 뒤 채택되어 다시 내려받지 않는다. | S-SHELL-ISO | Rust `isolated_dev_home_overrides_the_default_naia_dir`·`adopted_cascade_ready_matches_the_spawn_contract_shape` + `local-runtime.test.ts` ADOPTED 계약 핀 + Tauri 94 무회귀 |
| **FR-ADK-PATH.1** | Agent spawn `NAIA_ADK_PATH`/`NAIA_SETTINGS_DIR` 와 `SetWorkspace` 는 Shell adk-path 와 같은 루트다. adk-path 가 있으면 leftover `~/naia-adk` clone 경로를 쓰지 않고, 없으면 상속된 leftover env 를 제거한다. 기억·지식·음성 로컬 파일은 `<adk-path>/naia-settings` 아래에만 둔다. | S-ADK-PATH-ONE-ROOT | `lib.rs` `agent_spawn_env_uses_adk_path_not_leftover_home_clone` |
| **FR-RUNTIME.1** | 일반 `tauri:dev`/제품 실행은 native E2E 임시 workspace를 상속하거나 사용자 `~/.naia/adk-path`에 기록하지 않는다. E2E override는 명시적인 mode sentinel과 격리 runtime을 함께 요구한다. 일반 dev/build와 release staging은 동일한 paired-agent resolver를 사용하며, resolver는 `agent-pairing.json`의 exact `agentCommit`·`protoSha256`와 clean checkout, 필수 Agent entrypoint/proto를 모두 만족하는 candidate만 선택한다. 조건을 만족하는 candidate가 없으면 임의 sibling으로 폴백하지 않고 원인을 포함해 fail-closed 한다. | UC-SHELL-RECOVERY·UC-WIN-VOICE-6G | launch-env 단위 + Rust mode/cache 단위 + paired-agent resolver targeted coverage(candidate order, exact commit/proto/clean rejection, dev/build same selected root) + 일반 Tauri 로그 `SetWorkspace loaded=true` + E2E 전후 path-cache 불변 |
...
| **FR-KB-OS.7** | 컴파일 상태 — `read_naia_knowledge_kb({adkPath,scope})` 로 kb.json envelope(`{version,kb}`) 통계 표시. **검색 가능(`serveReady`=`status!==gap`)** 과 **검증(`accepted`=gold-QA)** 을 구분한다. 부재 = "미컴파일". 컴파일된 draft 카드는 검색 가능 수에 포함되며 accepted=0 이 빈 지식으로 보이지 않는다. | UC-KB-MANAGE | `knowledge-config.test.ts`(parseKbStats)·RTL |
...
| **FR-BGM.20** | sidecar는 루프백 두 주소(`127.0.0.1`, `::1`)에서만 받는다. 모든 인터페이스에서 받아 Windows 첫 실행에 방화벽 창을 띄우거나 같은 네트워크의 기기에 인증 없는 서버를 열지 않는다. IPv6가 꺼진 기계에서 `::1`을 못 열면 IPv4 루프백만으로 계속하고, `127.0.0.1`의 `EADDRINUSE`는 FR-BGM.14대로 치명이다 (#716). | Done | vitest `src/test/bgm-sidecar-loopback.contract.test.ts`: 127.0.0.1·::1 도달, LAN 주소 거부, 점유 포트 치명 오류 |
...
| **FR-AVATAR.3** | VRM/WebGL renderer 초기화 예외가 빈 캔버스나 Shell crash로 끝나지 않는다. 아바타 영역은 기존 현지화 오류 안내를 표시하고 준비 완료 상태를 거짓으로 표시하지 않아야 한다. renderer가 일부 생성된 뒤 설정이 실패하면 가능한 자원을 정리한다. 사용자가 다른 아바타를 선택해 `modelPath`를 바꾸면 초기화를 다시 시도하며, chat/settings는 계속 사용할 수 있어야 한다. | `AvatarCanvas.test.tsx` WebGL constructor/pixel-ratio exception·visible error·partial dispose·`modelPath` reinitialization regression + avatar scenario targeted coverage |
...
| **FR-NVA-WEB.5** | **Voice-gated talking loop (2026-09-24)**. While the shell TTS plays, a pre-baked NVA renderer shows its looping talking clip only while the RMS of the audio playing now is at or above 0.015, and returns to the idle clip (mouth closed) once it has stayed below that for 200 ms — the same rule and defaults as the naia.land Studio clip engine (`audio-gate.ts`), so one Studio `.nva` moves its mouth the same way on the web and in the shell. The level is measured from the audio data itself (streamed PCM chunks on the AudioContext clock, or a 16-bit WAV by `currentTime`); when it cannot be measured (MP3, browser speech) the renderer keeps the whole-playback talking loop as before. The idle loop keeps playing under the talking loop so the switch needs no `src` change (FR-VOICE.20 ⑤); it is only played, never paused or sought, and the talking loop is left without a pause when speech ends (FR-VOICE.23). Two shell-only additions (2026-09-25) keep a Studio avatar whose idle and talking clips are framed a little differently (head size, hair) from looking like its head jumps: once the gate closes it stays closed at least 250 ms (`NVA_SHELL_MIN_IDLE_MS`; the first word of an utterance still opens at once), because in the 2026-09-25 IR recording 71 of 205 idle stretches lasted under 250 ms and showed as one- or two-frame flashes; every switch of the drawn clip fades the previous clip out over the new one in 100 ms (150 ms left a visible double image of the glasses), reversing from the same mix when it switches back mid-fade (skipped for chroma-keyed clips, which share one keyer); and when speech ends, the idle clip stays on screen while the async switch back to the idle loop resolves, instead of the talking loop showing for about 300 ms at every page end (take 2 of the same recording). The web engine keeps its rule unchanged (the gate's minimum defaults to 0). | `voice-level.test.ts` envelope·WAV·timeline·queue level + `nva-voice-gate.test.ts` gate hold·shell minimum idle·crossfade mix and reversal·fading clip drawn over the new one·idle kept while speech ends·talking only while audible·unknown-level fallback·idle kept running + `prebaked-renderer.test.ts` |
...
| **FR-V017.2** | Startup auth, notification configuration, and every LLM/TTS credential are cached and sent after ADK setup and onboarding complete, including a clean install whose first mount had no config. Credential routing is destination-specific: when Nextain starts or reloads, an API key left in ADK config for a prior direct-provider must never be copied to `naia-anyllm`; only the selected Naia credential/reference may cross that boundary. Failure is observable and the request is not reported as successful. | UC mapping: UC12·S04·S54·UC-SHELL-RECOVERY; App component contract + `useAgentAuthSync` regression (stale direct-provider key is not sent to `naia-anyllm`, selected Naia auth is sent) + fresh-profile Playwright + Agent log redaction/inspection |
...
| **FR-V017.40** | 로컬 음성 내려받기 목록(`download-manifest`)은 운영체제마다 따로 고정한다. 저장소에는 Windows용 `scripts/voxcpm2-download-manifest.json`(windows_trt_6g 0.2.2 r2)과 리눅스용 `scripts/voxcpm2-download-manifest.linux.json`(linux_trt_6g 0.2.3 r1, 0.2.3 릴리즈에 실린 것과 같은 내용)이 있고, dev 런처·E2E 빌드·Rust debug 선택은 모두 `VOXCPM2_PROFILES`/`voice_runtime::host_os()`로 제 운영체제 파일을 고른다. 셸은 목록의 `profile`이 이 기계 운영체제용이 아니면 받기 전에 거절하고, 오류에 목록의 프로파일·대상 운영체제·이 기계 운영체제·파일 경로를 적는다. 리눅스 dev 셸이 Windows 목록을 읽어 2.5GB Windows 엔진을 받은 뒤 provenance 검사(python/bin/python3, http_server.*.so 없음)에서 실패하던 결함(2026-09-24)을 막는다. 같은 운영체제의 설치본은 FR-V017.38 핀(artifactManifestSha256)이 맞으면 다시 받지 않는다. | Rust `debug_scripts_pin_follows_the_host_platform`·`download_manifest_for_another_platform_is_rejected_before_download`, `stage-voxcpm2-runtime.test.ts`(운영체제별 고정·스크립트가 파일 이름을 직접 적지 않음), 변이 3종 확인 |
...
| **FR-CHAT-THINKING.3** | 설정 화면에 3개짜리 라디오 그룹(분절 버튼, `role="radiogroup"`, 각 항목 `role="radio"` 및 `aria-checked`, 좌우 화살표 키 이동)으로 생각 세기(끔/낮음/높음)를 제공한다. 기본값은 끔("off")이며 선택 즉시 `persistConfig({thinkingLevel, enableThinking: thinkingLevel !== "off"})`로 저장한다. 설정 탭 전체 저장(`handleSave`) 시에도 `thinkingLevel`과 `enableThinking`을 함께 저장하여 값이 유실되지 않는다. (#709) Status: Done | `SettingsTab.test.tsx` (라디오 선택, 키보드 좌우 화살표 이동, 즉시 저장 및 handleSave 전체 저장 시 두 필드 포함 검증), `packages/shell/e2e/thinking-settings.spec.ts` (실 UI 라디오 선택 및 영속) |
| **FR-CHAT-THINKING.4** | 이전 설정 호환 마이그레이션(`resolveThinkingLevel`). `thinkingLevel`이 유효한 값이면 그대로 쓰고, 없으면 `enableThinking === true`일 때 "low", 그 밖에는 "off"로 해석한다. 저장 시 항상 두 필드를 일관되게 함께 저장한다. (#709) Status: Done | `config.test.ts` (없음/true/false/유효값/잘못된 값 옮기기 및 저장 계약) |
| **FR-CHAT-THINKING.5** | 턴 요청마다 `thinking: { level }`과 `enableThinking: level !== "off"`를 항상 명시적으로 함께 전송한다(off도 포함). `isNewCore()` 경로와 레거시 `chat_request` 경로 모두 지원하며, Rust `agent_grpc.rs`는 유효 문자열("off"/"low"/"high")을 proto enum으로 변환하고 루트 core 어댑터(`shell-compat.ts`, `chat-bridge.ts`, `tauri/uc1.ts`)는 유효한 thinking 값을 유지 전달한다. (#709) Status: Done | `ChatArea.test.tsx` · `chat-service.test.ts` · `agent_grpc.rs` cargo test · `uc1-shell-compat.contract.test.ts` · `uc1-bridge.contract.test.ts` · `uc1-chat.contract.test.ts` · `e2e/thinking-settings.spec.ts` |
| **FR-CHAT-THINKING.6** | 에이전트 짝 값(agent pairing) 갱신 및 드리프트 방지. `packages/shell/agent-pairing.json`, `packages/shell/src-tauri/build.rs`(커밋, proto SHA256, 표식 줄 `optional ThinkingRequest thinking = 15;`), `.github/workflows/build-installers.yml`, `.github/workflows/self-trust-gates.yml` 4곳을 지정된 짝 에이전트 커밋(`5724389cb219ad41b6a1df5fbf2cbd6a2d48e2d6`)으로 일치시키고 이전 커밋 잔존이 없어야 한다. (#709) Status: Done | `src/test/agent-pairing-drift.contract.test.ts` · 저장소 전체 grep 검증 (0건 잔존) |
...
| **FR-VOICE-INSTALL-PRECHECK.1** | #700 | 로컬 음성 설치는 설치 프로세스(PowerShell/bash)를 띄우거나 대용량 패키지를 내려받기 전에 런타임 패키지 폴더·설치 스크립트·활성화 계약의 존재를 확인한다. 없으면 즉시 없는 경로와 해결 방법(`scripts/stage-voxcpm2-runtime.mjs` 로 스테이징)을 담은 오류를 돌려주고, 같은 문구를 설정·온보딩 진행 표시(voxcpm2_install_progress failed)와 naia.log 에 남긴다. E2E 모드에서 명시한 번들 폴더가 없거나 불완전하면 내려받기로 넘어가지 않고 부족한 항목을 나열해 실패한다. 파일이 모두 있으면 동작은 이전과 같다. 기동(start_voxcpm2)의 번들 없음 오류도 같은 형식이다. | Rust `voxcpm2_installer_precheck_tests` (폴더 없음·스크립트 없음·계약 없음·정상·다운로드 전 검사·E2E 명시 번들 오류·기동 전 순서) |
| **FR-VOICE-CACHE.1** | #703 | 세 층 분리: 게시 슬롯(공유) / 엔진 세대 `engines/<stamp>` / 모드별 상태. 준비 스크립트(ps1·sh)와 기동·판정은 자산 루트·엔진 디렉터리·상태 루트를 따로 받는다. | Rust `voice_cache` tests + `voxcpm2_installer_command_passes_asset_engine_state_roots` |
| **FR-VOICE-CACHE.2** | #703 | 하나의 준비 판정(`voxcpm2_readiness` → `evaluate_readiness`)을 설치 생략·상태·기동이 함께 쓴다. payload 만 있으면 스크립트를 돌린다. | `test_evaluate_readiness`, `test_gather_file_facts` |
| **FR-VOICE-CACHE.3** | #703 | 슬롯 키 = profile + artifact sha + (바이너리에 내장된) 준비 스크립트 sha + 활성화 계약 sha. 엔진 키 = GPU 이름·SM·드라이버·TensorRT 버전. 다른 스탬프는 새 세대로 게시하고 기존 세대는 옮기지 않는다. | `test_slot_key`, `test_engine_stamp`, `test_slot_record` |
| **FR-VOICE-CACHE.4** | #703 | 프로세스 간 잠금: 설치·게시·이전은 배타, 기동은 서버 수명 동안 공유. 잠금 파일에 소유 PID를 적고, 죽은 소유자의 OS 잠금은 자동 해제된다. 다른 인스턴스가 잡고 있으면 즉시 분명한 오류. :8910 을 이미 다른 프로세스가 쓰면 두 번째를 띄우지 않는다. | `test_lock`, two-home integration test |
| **FR-VOICE-CACHE.5** | #703 | 기존 모드별 런타임은 같은 볼륨이면 하드링크, 아니면 복사로 한 번 이전한다. pending·backup·state·hf-cache·로그·사용자 음성은 가져오지 않는다. 원본은 검증된 첫 기동 전까지 그대로 두고, 그 뒤에 중복된 자산 이름만 지운다. 공간이 부족하면 건너뛰고 알린다. | migration tests |
| **FR-VOICE-CACHE.6** | #703 | E2E 기본은 개인 쓰기 캐시, 사용자 캐시는 `NAIA_E2E_VOXCPM2_USER_CACHE=read-only` 일 때만 읽기 전용. 해석기는 LOCALAPPDATA 를 먼저 읽어 하니스 리다이렉트를 따른다. 정리는 junction 을 따라가지 않는다. | `test_resolve_cache_root`, `e2e-safe-remove.test.ts` |
| **FR-VOICE-CACHE.7** | #703 | release 빌드는 캐시·런타임 루트 재정의를 무시한다. 캐시 폴더는 사용자·SYSTEM 전용(DACL) / 0700, reparse point 거부, 기동 전 네이티브 모듈 해시 확인. | `test_resolve_cache_root`, `voxcpm2_runtime_root_ignores_override_in_release_semantics`, `test_native_hashes`, `test_reparse_and_ensure_private_dir` |
| **FR-VOICE-CACHE.8** | #703 | 보존: 프로파일마다 활성 슬롯과 직전 슬롯만 남기고, 공유 잠금이 걸린 슬롯은 지우지 않는다. Windows 준비 스크립트도 여유 공간을 확인한다. | `test_prune_slots`, `test_set_active_slot` |
...
| **FR-SLIDES-REC.1** | 슬라이드 발표를 MP4 로 녹화한다(ffmpeg — Windows gdigrab, Linux x11grab: FR-SLIDES-REC-LINUX.1, 산출 `<adkPath>/data-private/apps/land.naia.slides/video/`). 시작/정지 커맨드는 단일 활성 녹화만 허용(재진입 거부, poisoned-lock 방어). 프런트 라벨(`slides.recordStart/Stop`·`focusStart/Exit`·`notesShow/Hide`)은 14개 언어 i18n. | S-SLIDES-REC | `app_sandbox.rs` 녹화 상태머신 + i18n 생성 --check + 수동 실기 |
| **FR-SLIDES-ASSET.1** | 설치 앱 리소스 URL은 디렉터리 구분자를 보존하여 JS/CSS의 상대 참조가 같은 패키지에서 해석되어야 한다. 이전 설치본의 단일 세그먼트 URL도 앱 목록 로딩에서 복구하며 원본 HTML을 보관한다. 실제 패키지 내부 파일만 수정하고 기존 asset scope와 iframe origin을 유지한다. | UC-SLIDES-PDF-ASSET-RECOVERY | `app_assets_test.rs`; browser/native evidence distinguished |
| **FR-SLIDES-SIDECAR.1 (Pending)** | 사용자가 선택한 PDF의 정확한 동일 폴더·동일 basename `.md`를 자동 대본으로 우선 연결한다. 기본 파일 선택과 수동 대본 선택을 유지하며, 대본 없음/읽기 실패는 PDF 열기를 막지 않는다. 취소는 기존 문서를 보존하고 교체는 이전 대본을 제거한다. 임의 경로 IPC·권한 확대·새 변환 의존성 없이 설치 앱에서도 동작한다. | UC-SLIDES-SIDECAR | `slides_files_test.rs`, `slides-files.test.ts`, `slides-files-bridge.test.ts`, `slides-center-area.test.tsx`, `slides-sidecar.spec.ts` |
| **FR-SLIDES-REPEAT.1 (Pending)** | 기본 꺼짐의 반복 재생 토글을 제공한다. 켜진 경우 지정 범위의 마지막 페이지 정상 발화 완료 시 새 generation으로 지정 시작 페이지 발표를 요청한다. 범위 미지정 시 전체 PDF를 쓴다. 토글은 현재 발화를 끊지 않으며 취소·일시정지·중지·실패·지난 generation 결과는 반복하지 않는다. | UC-SLIDES-REPEAT | `slide-presenter.test.ts`, `slides-center-area.test.tsx`, `slides-sidecar.spec.ts` |
| **FR-SLIDES-VIEWER-CONTROLS.1 (Pending)** | 설치 Slides iframe에만 전체화면 권한을 위임하고 실제 전체화면 상태·종료·거부를 처리한다. 전체화면에서도 제어 버튼을 유지한다. 발표문 닫기·다시 열기는 발화와 대본 내용을 변경하지 않으며 키보드 및 좁은 화면에서 접근 가능하다. | UC-SLIDES-VIEWER-CONTROLS | `slides-center-area.test.tsx`, `generic-installed-slides.test.tsx`, `slides-sidecar.spec.ts` |
| **FR-SLIDES-RANGE.1 (Pending)** | PDF 페이지 안의 시작·종료 범위를 편집한다. 대본 로드 시 유효한 첫·마지막 대본 페이지로 초기화하고 대본이 없으면 전체 범위를 쓴다. 지정 종료에서 완료하거나 시작으로 반복한다. 수동 부록 이동은 가능하되 범위 밖 자동 발화를 하지 않는다. | UC-SLIDES-RANGE | `slide-presenter.test.ts`, `slides-center-area.test.tsx`, `slides-sidecar.spec.ts` |
...
|---|---|---|
| FR-SLIDES-HOST.1 | Exact frame/source/origin checked bridge shares locale and allowlisted theme tokens, with live updates preserving deck state. | Pending |
| FR-SLIDES-HOST.2 | Scoped recording bridge uses host ADK. No arbitrary paths, secrets or shell commands. Reject duplicate start and unrelated stop; dispose safely. | Pending |
| FR-SLIDES-PPTX.1 | Preserve PDF; native picker accepts PPTX and lazily invokes installed LibreOffice with an isolated profile and app-owned temporary directory. No automatic installation, upload, source writes or termination of existing Office processes. | Pending |
| FR-SLIDES-PPTX.2 | Cache by source content/identity, converter version/options. Validate output PDF and bound lifetime/size without deleting active jobs. Propagate progress, cancel, timeout and stable localized errors; reject stale results. | Pending |
...
| FR-SLIDES-PREFETCH.1 | 발표 중 쪽을 넘길 때 다음 쪽 낭독이 합성 시간만큼 멈추지 않는다. 슬라이드 앱은 지금 쪽을 읽는 동안 다음 쪽 낭독문을 셸에 미리 알리고(`naia:slide-presenter-prefetch`, iframe은 `naia-slides:prefetch`), 셸은 지금 쪽 문장이 모두 합성 대기열에 들어간 뒤에만 다음 쪽 앞 두 문장을 같은 한 줄 대기열(로컬 음성은 LocalVoiceScheduler)로 미리 합성한다. 그래서 음성 서버는 여전히 한 번에 한 요청만 받고, 미리 합성이 지금 쪽 문장을 앞지르지 않는다. 쪽이 넘어가면 같은 문장은 다시 합성하지 않고 바로 재생한다. 일시정지·이전 쪽·쪽 이동·발표 종료·질문·실패·발표문 수정 때는 미리 합성한 소리를 버리고, 발표 끝 쪽(rangeEnd) 뒤로는 미리 합성하지 않는다. 합성 실패는 지금처럼 그 문장의 실패 경로(일시정지)를 탄다. 근거: IR 녹화 2026-09-24 쪽 넘김 무음 8.5·7.2·6.6초. 시험: `sentence-pipeline-prefetch.test.ts`, `slides-center-area.test.tsx`, `slide-presenter-iframe-bridge.test.ts`. | Done |
| FR-SLIDES-FOCUS-EXIT.1 | 집중 모드로 발표하는 동안 '집중 모드 종료' 버튼은 포인터가 2.5초 멈추면 사라지고, 포인터를 움직이거나 키보드 포커스가 가면 다시 보인다. 버튼이 슬라이드 오른쪽 위(덱의 쪽 번호 자리)를 가리지 않게 하기 위함이다. 발표 중이 아니면 늘 보인다. 집중 모드에서 슬라이드는 위에 붙지 않고 세로 가운데에 놓인다. 시험: `slides-center-area.test.tsx`. | Done |
| FR-SLIDES-PAGE-GAP.1 | 발표 중 쪽의 낭독 요청이 셸에 도착한 시점(새 쪽이 화면에 보인 직후)부터 그 쪽 첫 소리까지 최소 간격을 둔다. 기본값은 500ms(루크 결정 2026-09-24)이고 상수 `SLIDE_PAGE_MIN_GAP_MS` 하나에 두며, 슬라이드 낭독 분할 설정과 같은 관례로 `VITE_NAIA_SLIDES_PAGE_GAP_MS`로 바꿀 수 있다. 실제 대기는 max(최소 간격, 첫 문장 준비 완료)이다. 미리 합성이 끝나 있으면 정확히 간격만큼 기다리고, 합성이 더 오래 걸리면 추가로 기다리지 않는다. 쪽 안의 둘째 문장부터는 다시 기다리지 않는다. 일시정지·쪽 이동·발표 종료는 오디오 큐를 비우면서 대기 타이머도 취소한다. 발표 시작·재개·반복 첫 쪽에도 같은 규칙을 적용한다. 시험: `slide-page-gap.test.ts`. | Done |
| FR-SLIDES-REC-LINUX.1 | 리눅스에서 "MP4 녹화"가 실제 파일을 만든다. X11·Xwayland에서는 셸 자신의 GDK 창 XID와 그 DISPLAY로 `x11grab -window_id`를 쓴다(제목 검색이 아니라서 다른 "Naia" 창과 섞이지 않고, 창을 옮겨도 따라가며, 합성 서버에서는 겹친 창이 있어도 셸 창 내용만 찍힌다). 셸이 순수 Wayland 창으로 떠 있으면 조용히 실패하지 않고 "X11 또는 Xwayland가 필요하다"는 오류를 낸다(PipeWire 화면 공유 포털은 아직 지원하지 않는다). 소리는 PulseAudio·PipeWire 기본 출력의 모니터(`@DEFAULT_MONITOR@`)를 녹음하고 `NAIA_RECORDING_AUDIO_SOURCE`로 바꾸거나 `none`으로 끌 수 있다. 윈도우는 ffmpeg가 기본으로 열 수 있는 출력 녹음 장치가 없어 영상만 녹화하며, 같은 변수에 DirectShow 장치 이름(예: Stereo Mix)을 주면 소리도 넣는다. ffmpeg 오류 출력은 버리지 않고 마지막 12줄을 로그에 남긴다. 시작 뒤 1.5초 안에 ffmpeg가 끝나면 시작 실패로 돌려 화면에 녹화 실패를 띄운다. 정지는 표준입력으로 `q`를 보내 MP4 색인을 쓰게 하고, 8초 안에 끝나지 않을 때만 강제 종료한다. ffmpeg가 녹화 중에 죽었거나 강제 종료된 경우 정지는 `recording_lost` 오류를 내고, 앱은 녹화 상태에서 빠져나와 다시 녹화할 수 있다. 시험: `slides_recording_test.rs`(인자·플랫폼 분기, 가짜 ffmpeg로 조기 종료·q 정지·시한 초과 강제 종료·녹화 중 사망, Xvfb 창과 격리 오디오로 실제 ffmpeg 녹화), `slides-host.test.ts`·`slides-host-bridge.test.ts`(recording_lost 처리). | Done |
| FR-SLIDES-REC-LINUX.2 | 녹화 시작 명령은 호출한 `Webview`에서 셸 창을 찾는다. 셸 주 창에는 기동 때 브라우저 앱의 자식 웹뷰가 붙으므로, Tauri는 이 창을 더는 `WebviewWindow`로 보지 않는다(`Window::is_webview_window`는 모든 웹뷰가 창 라벨과 같아야 참). `WebviewWindow`를 인자로 받는 명령은 본문에 들어가기도 전에 "current webview is not a WebviewWindow"로 실패한다(2026-09-24 녹화 셸: 로그에 `[slides-recording] start` 없음, video 폴더 없음). 어떤 Tauri 명령도 `WebviewWindow`를 인자로 받지 않는다. 시작·정지 실패는 `[slides-recording] start/stop failed: <원인>`으로 로그에 남고, 설치형 앱에는 `recording_failed: <원인>`(240자 이내, 제어 문자 제거)으로 전달되어 화면에 보인다. 오류 줄은 버튼 막대 위에 떠서 버튼 위치를 바꾸지 않는다. | Rust `command_argument_tests`(명령 인자 전수 검사), `slides-host(-bridge).test.ts`(원인 전달·로그), `slides-script-editor.test.tsx`(오류 위치), 격리 실앱 검증(Xvfb + 격리 PipeWire 널 싱크 + 개발 셸 + 설치형 슬라이드 앱: 옛 바이너리는 같은 오류 재현, 고친 바이너리는 앱 버튼으로 녹화 시작·정지 → ffprobe h264 1366x768 + aac 48kHz, 소리 -21 dB), 변이 확인 |

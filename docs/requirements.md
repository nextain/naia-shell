...
| **FR-NVA-WEB.5** | **Voice-gated talking loop (2026-09-24, revised 2026-09-25)**. While the shell TTS plays, a pre-baked NVA renderer shows its looping talking clip only while the RMS of the audio being heard is at or above 0.015 (naia.land Studio clip engine threshold) and shows the idle clip (mouth closed) in pauses. The level is read from the audio data itself: streamed PCM chunks on the AudioContext clock, or a 16-bit WAV on the `<audio>` element's `currentTime`; MP3 and browser speech keep the whole-playback talking loop. ① Pause rule (shell): a pause closes the mouth only when its whole length, counted in 20 ms windows on the audio clock (windows already heard + now + windows still to come), reaches 400 ms (`NVA_SHELL_HOLD_MS`); a shorter pause never closes it, at any display frame rate. Where the audio ahead is not known yet, the state holds until the silence already heard reaches 400 ms. The web keeps its 200 ms causal rule. ② Audible moment: AudioContext time minus `outputLatencySeconds` (a real `getOutputTimestamp()` reading, else `baseLatency + outputLatency`, capped at 1 s; the same function the AudioQueue start/audible timers use); `<audio>` `currentTime` as is (already the playout position); `play`/`playing` event timing is not used. The renderer reads 60 ms ahead of that moment (`NVA_LEVEL_LEAD_MS`, display compensation only). ③ Sentence boundaries: the sentence queued next is placed at the earliest moment it can be heard, bound to its item when the current one ends; once nothing is queued, held for ordering or reserved for synthesis, the time after the audio is silence, so the mouth closes as the voice ends. ④ Signals: the NVA talking loop starts on the coarse queue start (or earlier, from the level itself) and a coarse end waits until the gate has closed; a repeated start does not restart the loop. `onAudibleChange` (VRM mouth) turns off only after 400 ms of silence, or with the last sound when nothing more is coming. ⑤ Clips: the idle loop keeps playing under the talking loop; loops alternate two `<video>` elements (`TwinLoop`) and are never paused or sought while playing (WebKitGTK freeze); every clip switch crossfades over 100 ms. | `nva-voice-gate.test.ts` (window-count rule, 400 ms boundary, mid-pause, unknown ahead, 24/30/60/144 fps phase sweeps, soft stop, no restart, level start) + `voice-level.test.ts` (levelsAround, latency function 0/missing/large/getOutputTimestamp, sentence binding, page end, playback rate) + `audio-queue.test.ts` (audible hold) + `lipsync-platform-matrix.test.ts` (latency 0/0.04/0.15/0.3 s × 24/30/60/144 fps × play event ±100 ms: every mouth change within 0.1 s of the voice, no closure in a pause under 400 ms) + `twin-loop.test.ts` + `prebaked-renderer.test.ts`; Windows WebView2 and macOS WKWebView real runs not yet done |

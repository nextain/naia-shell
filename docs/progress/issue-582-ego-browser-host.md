...
- 장부는 둘로 나눈다. 영속 `targetId → workspace` 와 배타적 `targetId → {connection, generation}` **타깃 lease**. 배타는 attach 에 대한 것이다. 탭을 만든 연결은 이벤트 라우팅 주인으로만 기록되며, 같은 공간을 공유하는 다른 연결이 그 탭에 attach 할 수 있다(S2c 실측으로 확정). 감독자 자신의 CDP 요청은 장부 소유를 만들지 않는다. 같은 타깃에 두 연결이 동시에 attach 하면 원자적으로 한 연결만 승인하고 나머지는 원래 id 로 `EGO_TARGET_BUSY`. attach 요청마다 세대(generation)를 올리고, 응답이 오기 전에 `{connection, targetId, generation}` 으로 예약한다. 연결이 예약 중에 끊기면 예약을 철회하고, 그 뒤 도착한 늦은 응답은 감독자가 내부적으로 `Target.detachFromTarget` 한다. detach 된 sessionId 는 묘비(tombstone)로 남겨 같은 값이 재사용돼도 옛 세대의 요청·이벤트를 거부한다.
...
- **웹뷰에 노출하는 명령은 효과가 고정된 작업뿐이고, 등급은 Rust 가 명령 이름으로 정한다(S7).** S6c 의 다리는 임의 grant 로 토큰을 만드는 명령·아무 관리 RPC 나 부르는 명령·아무 등급 연결에서나 원시 CDP 를 보내는 명령을 웹뷰에 그대로 냈다. 등급 판정을 하는 쪽과 그 판정을 지나지 않고도 감독자에 닿는 쪽이 **같은 웹뷰**였으므로, 웹뷰가 한 번 뚫리면 등급표와 승인 장부가 통째로 장식이 된다. 그래서 웹뷰가 부를 수 있는 것은 `ego_host_op_*`(열기·이동·스냅샷·클릭·입력·평가·캡처·닫기·공간 만들기·공간 목록·공간 닫기) 뿐이고, 각 명령의 등급은 **그 명령의 이름**이 정한다(`ego_host_bridge.rs` 의 `OP_TIERS`, 서비스의 `BROWSER_RPC_TIERS` 와 같은 값임을 단위 테스트가 그 파일을 읽어 대조한다). 클라이언트가 실은 `grant`·`approvalRef` 는 받는 자리에서 버린다. 토큰 발급과 관리 채널은 Rust 안에서만 쓰이고, 관측 등급 작업은 grant 없이 붙어 원시 CDP 가 Rust 와 감독자 두 곳에서 막힌다. `script`(임의 heredoc)는 승인을 남기는 자리가 Rust 에 없으므로 어댑터와 Rust 양쪽에서 `approval-missing` 으로 거부한다 — FR-ENV-TOOL.14b(승인 UI)가 닫히기 전까지 웹뷰에서 heredoc 이 시작되는 길은 없다.
...
- 2026-09-10 Codex 구현 리뷰(gpt-5.6-sol, high, 코드 대상): 불가 판정 7건(P0 1·P1 5·P2 1). 웹뷰가 임의 grant 로 토큰을 받고 원시 CDP 를 보내 서비스 판정을 건너뛸 수 있던 것, 묘비 재사용·호스트 attach 예약 누수·핸드셰이크 덮어쓰기·자원 세션 소유 미검사·취소 장벽 귀속·컨텍스트 인자 타입. S7 에서 전부 수정하고 변이 탐침으로 확인. 첫 시도는 제공자 필터에 걸려 빈 출력이었고 정확성 리뷰로 표현을 바꿔 재실행했다.
- 2026-09-10 Fable 최종: 머지 가능. 실기 게이트 통과는 리눅스뿐이며 win32·darwin 은 기능 플래그 꺼진 채 게이트 열림. 후속 다섯 건은 process-status.json 의 open_followups.

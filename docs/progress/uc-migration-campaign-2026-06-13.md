...
## 권장 실행 경로 (다음 세션, 클린/재부팅 후)
1. 재부팅 → `pnpm run tauri:dev` startup 타임라인 재관측(logs-first) → 90s 블록 실제 점유자 격리·수정.
2. startup 클린 확인 후: **F2(workspace 관측 read-only)** 부터 Old-Baseline 측정(루크 게이트 불요) → 이식 → drift-gate. (F0/F1 은 일부 스캐폴드 존재, F2 가 외부의존 없는 첫 순수 transplant 슬라이스로 적합)

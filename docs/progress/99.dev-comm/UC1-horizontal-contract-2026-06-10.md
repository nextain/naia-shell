...
| `ChatTurn` | requestId 로 묶인 chunk 시퀀스 상태기계. **정상: `streaming → finish/error(terminal)`**. **취소: `streaming →(cancel 요청)→ cancelling → finish/error(terminal)`**. finish/error 는 streaming·cancelling 양쪽에서 종결(=ownership 해제, B.2 일치). ⚠️ cancel 은 비종결 요청. 순수. |

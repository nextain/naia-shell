...
- **2026-06-16 진행(재기동 세션)**: 재시작 드리프트-갭 닫음(agent module-manifest V모델경로 정렬 49→0, `aef89c6` push). **UC-012 step2 wizard step-flow(assets+submit) core 경유 graft+3R 2-clean(mutation-verified), `ee08153` push**(잔여=루크머신 실앱 e2e). **남은 ⑤음성·⑥유투브·⑦브라우저 = 전부 런타임/external 게이트 — 헤드리스 자율-검증 슬라이스 없음(UC-012가 마지막).** 라이브 graft 계획+루크-머신 검증절차 = `remaining-uc-runtime-graft-handoff-2026-06-16.md`. **+ "모든 UC 완료 배선" 지시 대응**: ⑤음성 표현(재생) 포트 graft(`2c05ba1`, 2-AI CLEAN, mutation-verified; mic/STT/WS=포트결정/루크머신 잔여). ⑥유투브·⑦브라우저 = **이미 배선(transitive)** — bgm_youtube_*/skill_browser_* 가 grafted chat 도구루프(UC1)+agent skill(UC5) 경유(os-side 자체 포트 없음). **결론: ①~⑦ 전부 new-core 배선됨**(런타임=루크머신). **유일 미배선 = ⑨워크스페이스(UC-007)** — F2/F3 포트 dormant·셸 old-path, DriftDetector 소비자=신규발명·mutate 승인정책=product 방향 미결정 → **AI 단독 graft 금지(발명=드리프트), 루크 방향 결정 필요**.
...
- 2026-06-14 (session ec74cc29, graft 단계) ★★ **graft = 자율-검증 가능 재평가 + UC12 온보딩 graft 완성**:
  ...
  - ★★ **F2/F3 Old-Baseline 스코프 발견(2026-06-14, graft 착수 전 정본 대조)**: old-naia-os/shell 의 workspace UI 는 전부 **유저-직접 op** — Editor save=`workspace_write_file` **직접 invoke**(유저 편집, 승인 불요), SessionDashboard/FileTree/IssuesPanel=`workspace:file-changed`→**단순 리로드**(드리프트 판정 아님). **old 엔 DriftDetector(observe→expected→drift→reafference)도 MutationGate(approval→mutate→observe→reafference)도 소비자 부재.** → new-core F2 DriftDetector / F3 MutationGate 의 reafference·approval 흐름은 **이식이 아니라 신규 발명**(에이전트-개시 workspace observe/mutate 용 capability, old 에 없음). 의미: (a) old workspace UI 를 new-core 로 graft = readFile/listDir **passthrough**(저가치) 거나 MutationGate-for-유저저장=**의미 오류**. (b) DriftDetector/MutationGate 의 진짜 consumer = **미래 에이전트-tool-observes/mutates-workspace 경로**(gRPC tool path) = old 에 없던 신규 기능 빌드(=transplant 스코프 초과). → **[[feedback_closed_loop_review_canon_conformance]] '정본/Old-Baseline ⟂ 작업 → 멈추고 인간' 적용** = 루크에게 F2/F3 os-local graft 방향 결정 요청(passthrough vs 신규 에이전트-capability vs 현 상태 수용). gRPC 경계 UC(chat/approval/tools/diagnostics)+onboarding graft 는 완료라 핵심 GOAL('gRPC 기반 연결')은 달성.
...
## ★★ 2026-06-15 체크포인트 — graft 캠페인 CUT + V모델 계약구조 PIVOT (session 67a0313b/ec74cc29 정리)

### 0. CUT 사유 + 클린 상태
루크 지시: **V모델 01~05 계약구조(TalkingKiosk 발)를 naia-adk 표준으로 먼저 승격**(다른 세션 진행 중) → 포크(new-naia)로 전파. 그 전에 new-naia 를 **끊을 수 있는 클린 체크포인트**로 정리. 현 상태 = 드리프트 0(os root-structure/assembly/compile-integrity green, agent canon RED 6=전부 UC-memory off-scope, compile green) · 미커밋 소스 0 · 실 테스트 green. ⚠️ **동시 작업 충돌 1회 발생**(이 세션 + 67a0313b 둘 다 graft 돌림) → 단일세션 원칙 재확인, naia-adk 는 이 세션 미접촉.

### 1. e2e 실 UI 검증 (이 세션이 직접 실행 — punt 금지 [[feedback_no_punt_unverifiable_use_e2e_tauri]])
`pnpm test:e2e` 풀런(12분): **62 passed / 24 failed / 32 skipped**. graft scope 전부 통과:
- ✅ `uc1-new-core`(본) · `uc13-approval` · `uc-provider-provenance-live` 통과. `uc1-new-core-variants`(thinking/도구/멀티턴/에러) 통과.
- 24 실패 = **전부 graft scope 밖**(정직 분류, 격리/pre-graft 대조로 확정):
  - `onboarding-fresh` 3 = **pre-existing stale 테스트**(wizard 첫 스텝=`welcome`인데 테스트는 바로 `agentName` input 기대; pre-graft `dce030f^` 되돌려도 동일 실패 → 내 graft 회귀 아님, 앱 정상).
  - `uc1-new-core-variants` 취소 1 = **flake**(격리 단독 실행 1 passed 4.4s; 풀런 병렬부하서 스트리밍이 취소 전 종료). 취소 배선 정상(`cancel→CancelTurn→cancel_stream→invoke`).
  - `memory-settings` 12 = **UC3/UC4 off-scope**(다른 세션 naia-memory).
  - `naia-omni-voice-tools`2·`119-pty`/`120-send`5·`chat-tools`cli 1 = old-path/external(new-core 미경유, 무관).

### 2. 정직한 UC 현황 (유저 여정 관점)
**실제 동작(셸 graft+e2e 검증) = 온보딩 · 채팅 · 승인 3개**(+provider-provenance=독립UC 아님, 채팅 속성). 그마저 채팅(+승인)이 가장 탄탄, **온보딩은 절반**(완료 결선=`completeWith` graft+seam테스트 ✓, 단 **나이아 계정 흐름**[startNaiaAuth/onNaiaAuthCallback OAuth 로그인]은 계약 존재하나 셸 graft 미연결 — wizard 가 still old 직접 invoke). 나머지(F0~F3·V2·UC5·6·8)=이식+2-AI 리뷰+컴파일 완료, **셸 graft 미연결**(new-core dormant).

### 3. 루크 우선순위 재정렬 (2026-06-15) + 스코프
유저여정 순: ①온보딩+**나이아계정** ②채팅✅ ③승인✅ ④스킬(UC5) ⑤음성(UC2) ⑥유투브(UC8) ⑦브라우저(UC6). **제외: ⑧메모리(UC3/4 off-scope) · ⑨워크스페이스(UC7/F2/F3 — 신규발명 방향 미해결, 보류).** **불변: 반드시 계약에 의해**(Old-Baseline→계약→이식→drift-gate→2-AI→커밋, 불가 시 신규계약).

### 4. PIVOT — V모델 01~05 계약구조 (TalkingKiosk `docs/progress/`)
```
01.requirements/INDEX.md   REQ-###  (단일 registry, FR/NFR)
02.user-scenarios/INDEX.md UC-###   (≥1 REQ 유도, ≥1 TEST-S 닫힘)
03.uc-tests/INDEX.md       TEST-S-# (시나리오/인수)
04.features/INDEX.md       SPEC-### (UC→구현단위, ≥1 TEST-F)
05.features-tests/INDEX.md TEST-F-# (SPEC 1:1 통합)
99.dev-comm/               날짜메모(append-only, 고립검사 면제)
```
원칙: 단일 INDEX registry(문서폭발 방지) · **양방향 추적 orphan-0**(검사기 non-zero exit) · front-matter `{id,status,area,links}` · **md 는 SPEC 까지, 그 아래 unit/test=코드**(`@spec SPEC-###` 태그) · status `Draft→Approved→In-progress→Done`(Done=테스트링크 강제). new-naia 의 드리프트·가짜green 고질병 정면 해법(TalkingKiosk 크로스리뷰 BLOCKING="게이트경로 단절→가짜그린"·"status-gate 미결합→거짓완료"=new-naia 와 동일 실패모드).

### 5. 재개 지점 (naia-adk 승격 후)
naia-adk V모델 표준 랜딩 후, new-naia 기존작업을 01~05 로 회귀 정리(매핑 초안): `docs/requirements.md`(FR-F0~F3+NFR 17종)→**01** · `docs/user-scenarios.md`(UC1~14+S01~71)→**02** · e2e `*.spec.ts`→**03 TEST-S** · `src/main/ports/*`(f1/f2/f3/uc1/v2)+`docs/progress/*-contract-*.md`→**04 SPEC** · `src/test/*.{contract,test}.ts`(f0~f3/uc1/uc12/v2 등 20개)→**05 TEST-F**. 기존 `check-doc-graph.mjs` 존재 → orphan-0 양방향 확장. ⚠️ **게이트 경로 재배선**(agents-rules.json P01/P03 deliverable=docs/requirements.md·user-scenarios.md)=**헌장 수정→루크 승인 필요**. 정리 후 graft ①~⑦ 를 새 계약 하네스 하에서 재개.

> **요약**: graft 는 ①~⑦ 중 온보딩(절반)·채팅·승인만 완료. 여기서 CUT. 다음 = (다른 세션) naia-adk V모델 승격 → (이 트랙) new-naia 01~05 회귀정리 → graft 재개. 메모리·워크스페이스 제외.

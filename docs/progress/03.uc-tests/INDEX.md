...
| TEST-S-024 | UC-GROK-SUBSCRIPTION, FR-GROK.1~4 | 설정에서 Grok 구독 CLI 선택, API 키 없음, 준비됨/로그인필요/설치필요/확인실패 구분, 재시도, 계정·CLI 출력 비노출, xAI 키 경로와 게이트웨이 `grok:` prefix 분리, 키 없는 chat_request | Playwright UI E2E | `packages/shell/e2e/grok-readiness.spec.ts` | Pass (8/8, 2026-09-03) |
| TEST-S-027 | UC-026, UC-DEVICE-PAIR, FR-DEVICE.1~4 | 설정 디바이스 섹션 표시 + pair/verify/approve 후 rotate가 이전 토큰을 무효화하고 rename/describe/revoke가 상태 변화를 만든다 | native+unit | `packages/shell/e2e-tauri/specs/43-device-operations.spec.ts`, `34-device-pairing.spec.ts`, `packages/shell/src/lib/__tests__/device-pairing.test.ts`, `packages/shell/src-tauri/src/device_pairing.rs` | Partial (unit/cargo pass; native e2e pending) |
| TEST-S-025 | UC-GROK-SUBSCRIPTION, FR-GROK.2 | 로그인된 실제 Grok CLI가 Brain 설정에서 준비됨으로 표시 | native Tauri E2E | `packages/shell/e2e-tauri/specs/96-grok-readiness.spec.ts` | Pass (1/1, 2026-09-03) |
...
| TEST-S-023 | UC-024 | AppBar 고정 순서와 웹 이동, 신뢰 origin·1회 state 딥링크, 단일 앱 확인, Shell 내 구매 부재, native 권리 재확인·ZIP 무결성 검증·설치 계약을 검증 | unit + Rust + Playwright 통합 | `AppBar.test.tsx`, `AppInstallDialog.test.tsx`, `app-store-client.test.ts`, app.rs/lib.rs store tests, 앱스토어 E2E | In-progress |
| TEST-S-026 | UC-025 | 격리 행렬 negative, 무간섭 세 겹, 헤드리스 인계 거부, 감독자 SIGKILL 뒤 Chromium 소멸과 조정, ADK 전환 순서, 승인 없는 묶음 실행 거부, 기존 임베디드 브라우저 도구 불변을 검증 | node:test 실브라우저 + vitest 계약 + Playwright + e2e-tauri | `packages/ego-host/test/*.test.mjs`, `src/test/env-tool-browser-host.contract.test.ts`, `packages/shell/e2e/env-tool-browser-host.spec.ts`, `packages/shell/e2e-tauri/specs/env-tool-browser-host-lifecycle.spec.ts` | Pass (linux, 2026-09-10; win32·darwin 게이트 열림) |

...
| TEST-S-017 | UC-018 | Codex provider 선택 UI, role별 설정 저장/복원, apiKey 없는 Codex chat 전달, Agent 정확한 commit/proto pair 강제를 계약·UI 테스트와 실제 Codex smoke로 검증 | 계약+통합+실 smoke | `packages/shell/src/components/__tests__/SettingsTab.test.tsx`, `packages/shell/src/lib/__tests__/adk-store.test.ts`, `chat-service.test.ts`, `src/test/uc-wire-v1-paired-proto.contract.test.ts`; Agent Codex smoke | Pass |

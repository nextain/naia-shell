...
| SPEC-018 | UC-025 | ego-lite 공개 런타임 벤더링(고정 커밋·매니페스트·동기화 스크립트·실행 ABI 문서), 셸 소유 감독자(Chromium 파이프 소유·lease·연결별 CDP 중계·기본 거부 행렬·격리 컨텍스트 장부·취소 훅·접근성 스냅샷·캡처), #499 실제 어댑터와 조립, 파생 스킬과 사이트 학습, 두 진입점 권한 등급, 기능 플래그와 기존 도구 분리 | core+ego-host+shell+Rust | In-progress | TEST-F-018 |

...
| REQ-015 | 전주대 Codex 준비 | Codex CLI 설치·로그인 상태를 안전하게 판정하고 수업 워크스페이스를 변경 없이 확인 | Pass | UC-019 | SPEC-013 | TEST-S-018 |
...
| REQ-020 | 앱 스토어 | AppBar는 바탕화면→브라우저→앱스토어→작업공간 순서를 고정하고 앱스토어를 내장 브라우저로 연다. 목록·상세·구매는 웹이 담당하며, Shell은 신뢰한 웹의 딥링크에서 단일 앱 ID만 받아 권리를 재확인하고 검수된 앱을 설치한다. 딥링크에 artifact URL이나 결제 기능을 두지 않는다. | In-progress | UC-024 | — | TEST-S-023 |
| REQ-022 | 로컬 디바이스 페어링 | 설정과 Tauri IPC로 pair/verify/approve/describe/rename/rotate/revoke. 게이트웨이 skill_device 는 복구하지 않는다. 회전한 토큰은 이전 토큰을 무효화한다 (#570). | In-progress | UC-026 | SPEC-019 | TEST-S-027 |
| REQ-021 | 에이전트 브라우저 호스트 | 에이전트 전용 헤드리스 브라우저를 셸이 소유한 감독자로 제공한다. 작업 공간은 비로그인 격리 컨텍스트이고 사람의 창·포커스를 건드리지 않으며, 셸 종료·강제 종료·ADK 전환 뒤 브라우저가 남지 않는다. 상세 FR 은 `docs/requirements.md` FR-ENV-TOOL.2a·10~15, NFR-ENV-TOOL-VENDOR.1·ABI.1. | In-progress | UC-025 | SPEC-018 | TEST-S-026 |

...
| `DomainOutbound` | **app→agent 송신 domain 의도 폐쇄 union**(AgentTransportPort.send 경계 타입; adapter 가 이를 AgentOutbound(protocol)→wire 로 변환). = `ChatRequest(위) \| CancelTurn{requestId,clientId} \| ApprovalResponseIntent{requestId,toolCallId,decision:'approve'\|'reject'} \| CredsUpdate{provider,secret:{apiKey?\|naiaKey?}}`(UC1 범위 — secret 은 도메인 내부에선 보유하되 *송신 채널만* creds_update 로 분리, F0 정합). AgentOutbound 4 variant 와 1:1 대응(domain↔protocol 매핑 = adapter). |
...
| `MessageRouter` | (AgentTransportPort.onMessage 단일 구독) | **demux 라우터**: AgentMessage(protocol) 전 variant switch → chat-turn(requestId)→**domain ChatChunk 변환 후 `ChatPort.deliverChunk`**(ports 계약, ChatService 구현=domain, 소유권 필수 — adapter→ports canon) / 비-chat→해당 semantic port(ToolPort 등) / UnknownAgentMessage·소유권 없음→**DiagnosticSink(error+log, 소유권 불요)** (STRUCTURE:215~221 canon, app 은 demux/protocol 안 봄). transport(wire)와 분리=단일 수신경로·중복전달 방지. |

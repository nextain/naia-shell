...
AgentOutbound = **shell→agent 송신 protocol DTO union**, baseline wire 와 동일하게 `type` discriminant 로 판별(`ChatRequestPayload{type:"chat_request",...} | CancelPayload{type:"cancel_stream",requestId} | ApprovalResponsePayload{type:"approval_response",...} | CredsUpdatePayload{type:"creds_update",...} ...`). ⚠️ transport 는 payload-agnostic — 전 outbound 가 같은 send 경로(type 별 분기는 wire encode·agent 측). AgentMessage(수신, type discriminant)과 대칭.
...
TurnHandle = { requestId, clientId, unsubscribe }   # (startTurn 은 {handle, sent:Promise} 반환)   # 핸들(위조 방지 = 아래 레지스트리가 권위, 핸들 단독 아님)
  # ⚠️ ownership 레지스트리(ClientSessionPort/adapter): startTurn 시 requestId→clientId 등록 + **충돌 거부**(중복 requestId) + **해제 = inbound 종결 chunk(finish/error) 수신 시 OR send reject(초기 send·cancel_stream 전송 실패=transport 단절, 후속 chunk 없음)**. ⚠️ **cancel 은 *요청*이라 즉시 해제 안 함** — cancel_stream 후 뒤따르는 finish/error 가 해제(즉시 해제하면 trailing chunk 라우팅 불가, R10). *(client lease 만료=ClientSessionPort UC10a 범위, 본 UC1 turn-terminal 아님 — baseline 은 chat-turn timeout 없음)*. legacy agent_response 엔 clientId 없으므로 inbound 측은 requestId 로 레지스트리 조회해 clientId 복원. cancel/approval 권한 = 레지스트리(=ClientSessionPort)가 소유주 대조해 인가(타 client 차단)
...
| 어댑터 | 포트 | 구현 |
|---|---|---|
| `StdioTransportAdapter` | AgentTransportPort | **순수 wire 변환만**: `AgentOutbound→wire JSON-line encode`(chat-request·cancel_stream·approval-response·creds 등 전 outbound) / `wire→AgentMessage union decode`. demux·라우팅 안 함(=MessageRouter). `send_to_agent_command`(stdin)+`agent_response`(stdout). ⚠️ **flat newline JSON 만**(agent 는 한 줄 곧바로 parseRequest). protocol-bridge StdioFrame v1=미사용 scaffold라 *보내지 않음*. gRPC=후속 어댑터(envelope 그때, AgentTransportPort 교체만). |
| `MessageRouter` | (AgentTransportPort.onMessage 단일 구독) | **demux 라우터**: AgentMessage 전 variant switch → chat-turn(requestId)→**`ChatPort.deliverChunk`**(ports 계약, ChatService 구현, 소유권 필수 — adapter→ports canon) / 비-chat→해당 semantic port(ToolPort 등) / UnknownAgentMessage·소유권 없음→**DiagnosticSink(error+log, 소유권 불요)** (STRUCTURE:215~221 canon, app 은 demux 안 함). transport(wire)와 분리=단일 수신경로·중복전달 방지. |

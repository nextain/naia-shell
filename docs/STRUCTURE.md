...
| `AppPort`(facade: **Chat·Tool 만**) | bi | ports | **conversation·tool interaction 전용**(범용 이벤트 허브 아님) | 1단계 골격 |
...
- **interaction substrate 배치(R2 MED)**: 실시간 turn-taking timing = **app 레이어** 소유, `AppPort.Chat`로 노출. provider(모델)와 분리 — provider=무엇을, interaction=언제(타이밍).
- **ingress 소유 경계(R5 codex HIGH) — Chat vs Sensory 중복 제거**: `SensoryPort` = **raw 감각 신호**(오디오 파형·화면·관측 텍스트) → perception(STT·prosody·화자). `AppPort.Chat` = **대화 상호작용**(의미 턴 + turn-taking 조율), raw 오디오를 먹지 않음. → 음성 발화: *오디오*는 Sensory(청각)로 들어와 perception, *대화 턴 구조/타이밍*만 Chat. 텍스트: 사용자 *직접 입력* = Chat / 화면·환경 *관측* 텍스트 = Sensory. (구체 ingress 매핑 = 2단계 시나리오.)

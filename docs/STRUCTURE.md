...
> 상태: **R1 적대 리뷰(codex·gemini) 반영 완료 — HIGH 4·MED 3 교정. Q3(gateway) 루크 결정 + R2 클린 확인 후 승인.** 승인 전 슬라이스 이식 금지.
...
- **shell**: 외관 유지(75K, 리팩터 안 함). 이식 후반에 **verbatim 편입**(레이어 분해 대상 아님). 편입 형태(packages/shell 등)는 아래 미해결.
...
- **Q3 gateway·voice-server = 갈림 ⚖️ (루크 결정 필요)**: codex = **별도 프로세스 유지**(지금 라이브러리로 흡수하면 GatewayPort 의미 조기 고정, gRPC 설계 편향). gemini = **라이브러리 슬라이스로 흡수**(1단계 = clean 이식, 프로세스 분리는 4단계에). → 1단계 닫기 전 루크 택1.

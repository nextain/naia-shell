# naia-shell 이식 구조 (1단계 산출물 — 구조 뼈대) v5 (인지흐름 재그룹핑)

> **이름 정리**: 이 문서의 `naia-shell` = 이 레포의 비주얼 셸 코드베이스(이식 대상). `naia-os`(있다면) = 이 셸을 부팅 이미지로 묶는 Bazzite/titanoboa 배포판 계층 = 별개 관심사. `old-naia-os`/`new-naia-os`/`@nextain/naia-os-core` 는 이식 당시 디렉토리·패키지 식별자라 그대로 둔다.

`[Phase 01]`

> 추적: L0 미션(naia-shell 전체를 헥사고날 gRPC로 이식) / 진행 단계 **1**.
...
> 슬라이스가 "기능 단위"처럼 보이지만, 실제 추상화 축은 *기능*이 아니라 **AI(뇌) 중심**이다. naia-shell 이 장기적으로 **피지컬 로봇**까지 본다면, 데스크톱 앱 기능 단위로 굳는 순간 그 사상을 표현할 수 없다.
...
8. 이식하며 얻은 **일반화 패턴만** template 로 환류. **"Template Reusable Unit" 판정 (R1 gemini MED 수정)**: naia-shell 전용 식별자·도메인 결합이 없는지 검증 통과분만 환류(template 오염 방지).

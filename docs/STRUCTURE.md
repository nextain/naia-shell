...
| `InteroceptivePort`(신설, C1 수렴) | afferent(in) | ports | **내수용/자기상태**(배터리·시스템 부하·로봇 pose/관절/열·자기 처리상태) → perception/affect 기반 | 2단계 |
...
**efferent 계약 원칙(C1 필수 — physical-AI 先占)**: 모든 출력 포트(Express/Action)는 **async + ack/result-feedback + interruption** 계약. 아바타=즉시 렌더지만 로봇 액추에이터=지연·중도중단 → 의도 emit + **결과 피드백(efference copy)** 경로 필수(없으면 경험이 "내가 무엇을 했는지" 못 묶음, 진행 중 명령 덮어쓰기 사고). 동기 가정 하드코딩 금지.

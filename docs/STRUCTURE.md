...
| **공간/주변**(배경화면·BGM = 아바타가 사는 공간) + **앱 surface**(browser·workspace·note·terminal) | **환경(제3)** | **양방향**: 관측(거주 공간·앱 상태)→경험 + 행위(공간/앱 툴) |
...
```
 ...
 │  마이크·화면·키보드    아바타VRM·스피커          배경·BGM·browser·앱     │
 └──────┬───────────────────────▲───────────────────────┬───────────────┘
...
3. **제3의 환경 (third environment) = naia 가 거주하는 공간** — 배경·BGM(공간 분위기) + 앱 surface(browser·workspace·terminal). **차원/substrate 무관(body-swap 과 동형)**: 2D 패널·배경(now) → **3D 환경 UX** → 로봇=물리 공간. `EnvironmentPort` 는 차원 무관 관측+행위(2D 패널 가정 하드코딩 금지). 각 앱은 tools/skills 를 agent 에 등록 → agent 가 LLM 에 노출 + 호출 라우팅(`PanelPort`: `panel_tool_call` ↔ `PanelToolResult`). **agent 가 매개자** — 환경이 LLM 에 직접 붙지 않음(코드 실측: `panelSkillsByPanel`·`pendingPanelToolCalls`).
...
| `SensoryPort` | afferent(in) | ports | **raw 감각**(audio 신호·vision·screen·관측 텍스트) → perception. *대화 텍스트 아님* | 2단계 |
| `ExpressionPort` | efferent(out) | ports | embodiment-neutral 표현 의도 | 2단계 |
| `EnvironmentPort`(상위) ⊃ `PanelPort`(패널 사례) | bi | ports | 환경 관측 + 행위(`panel_tool_call`↔`PanelToolResult`) | 1단계 골격 |

    ...
    std::thread::spawn(move || {
        ...
        loop {
            ...
            if let Ok(state) = CHROME.try_lock() {
                if state.pid == pid
                    && state.chrome_handle.is_valid()
                    && state.last_client_rect.width > 0
                {
                    if state.overlay_mode && state.chrome_visible && state.tauri_handle.is_valid() {
                        let (th, ch, rect) = (
                            state.tauri_handle,
                            state.chrome_handle,
                            state.last_client_rect,
                        );
                        drop(state);
...
fn agent_browser_bin() -> Option<String> {
    ...
    if let Ok(exe) = std::env::current_exe() {
        if let Some(dir) = exe.parent() {
            let bundled = dir
                .join("agent/node_modules/agent-browser/bin")
                .join(native);
            if bundled.exists() {
    ...
    {
        let nvm_home =
            std::env::var("NVM_HOME").unwrap_or_else(|_| format!("{home}\\AppData\\Roaming\\nvm"));
...
fn spawn_chrome(port: u16, tmpdir: &str) -> Result<Child, String> {
    ...
    let bin = wm
        .chrome_bin()
        .ok_or("Chrome not found (searched PATH and well-known locations)")?;
...
pub fn browser_embed_init(
    ...
    if std::env::var("NAIA_DISABLE_BROWSER_EMBED").is_ok() {
        crate::log_both("[browser] init skipped — NAIA_DISABLE_BROWSER_EMBED is set");
    ...
    if state.process.is_some() {
        ...
        crate::log_verbose(&format!(
            "[browser] re-embed: chrome_handle={chrome_handle:?}"
        ));
        if is_overlay {
    ...
    let chrome_handle = match platform::find_new_chrome_window(&chrome_baseline, 6000) {
        ...
        Err(e) => {
            crate::log_both(&format!(
                "[browser] diff lookup failed: {e} — fallback to PID lookup"
            ));
            wm.find_window_by_pid(pid, 6000)?
        }
    };
    ...
    if let Some(bin) = agent_browser_bin() {
        ...
        match out {
            ...
            Ok(o) => {
                ...
                crate::log_verbose(&format!(
                    "[browser] agent-browser failed ({}) — fallback to CDP: {stderr}",
                    o.status
                ));
            }
...
pub fn browser_embed_page_info() -> Result<(String, String), String> {
    ...
    let resp = ureq::get(&url_api)
        .call()
        .map_err(|e| format!("CDP /json/list: {e}"))?;
    let tabs: Vec<serde_json::Value> = resp.into_json().map_err(|e| format!("JSON parse: {e}"))?;
    let tab = tabs
        .iter()
        .find(|t| t["type"] == "page")
...
pub async fn browser_open_login(app: AppHandle, url: String) -> Result<(), String> {
                ...
                if current.contains("/desktop/auth-complete") {
                    if !auth_emitted {
                        if let Ok(parsed) = current.parse::<url::Url>() {
                            ...
                            if !key.is_empty() {
                                let auth =
                                    serde_json::json!({ "naiaKey": key, "naiaUserId": user_id });
    ...
    let chrome_path = if let Some(p) = chrome_for_testing_bin() {
        ...
        let bin = agent_browser_bin().ok_or("agent-browser not found — reinstall Naia")?;
        crate::log_both(
...
pub fn browser_embed_kill() {
    {
        ...
        state.pid = 0;
        state.chrome_visible = false;
        state.last_client_rect = WindowRect {
            x: 0,
            y: 0,
            width: 0,
            height: 0,
...
mod tests {
    ...
    fn returns_none_when_key_missing() {
        let body = make_tab_list("https://naia.nextain.io/desktop/auth-complete?user_id=user-42");
        assert!(parse_auth_complete_from_tab_list(&body).is_none());
    }
    ...
    fn returns_none_when_key_is_empty_string() {
        let body =
            make_tab_list("https://naia.nextain.io/desktop/auth-complete?key=&user_id=user-42");
        assert!(parse_auth_complete_from_tab_list(&body).is_none());
    }

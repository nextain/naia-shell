 ...
 *   1. Subscribes to a `PanelContext` source (typically `usePanelStore`).
...
export interface PanelContextSource {
	/**
	...
	getContext(): PanelContextUpdate | null;
}

export interface PanelContextBridgeOptions {
	 ...
	 * the current `activePanelContext` into `buildMemoryContext()` →
...
export interface PanelContextBridge {
...
/**
 * Attach a debounced bridge from a `PanelContextSource` to a `VoiceSession`.
 *
 * The bridge:
 ...
 *     payloads are < 1 KB).
 *
 * @returns a `PanelContextBridge` handle. ALWAYS call `detach()` from the
 ...
 */
export function attachPanelContextBridge(
	session: VoiceSession,
	source: PanelContextSource,
	options: PanelContextBridgeOptions = {},
): PanelContextBridge {
	...
	let pending: PanelContextUpdate | null = null;
	...
	function dispatch() {
		...
		try {
			...
			Logger.warn("PanelContextBridge", "sendContextUpdate threw", {
				error: String(err),
				type: ctx.type,
			});
		}
	...
	function schedulePending(ctx: PanelContextUpdate) {
		pending = ctx;

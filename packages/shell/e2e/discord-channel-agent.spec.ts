	...
	window.__TAURI_INTERNALS__.invoke = async function(cmd, args) {
		...
		if (cmd === "discord_inbox_snapshot") return [
			...
			},
			{
				...
				participation: "all",
				unread: 2,
				lastActivity: 1719999998000,
				records: [{
					recordId: "incoming:m2",
					direction: "incoming",
					bindingId: "discord_100_202",
					guildId: "100",
					channelId: "202",
					sourceMessageId: "m2",
					authorId: "301",
					content: "지원 채널 전용 메시지",
					createdAt: 1719999998000,
				}],
			},
		];
...
test("Channels 메시지는 개인 채팅에 복사하지 않고 읽음 상태만 저장한다", async ({
	...
	const privateTranscript = page.locator(".chat-messages:visible");
	await expect(privateTranscript).not.toContainText("배포 상태를 확인해줘");
	...
	await expect(privateTranscript).toBeVisible();
	await expect(privateTranscript).not.toContainText("배포 상태를 확인해줘");
	await expect(privateTranscript).not.toContainText("이전 Discord 대화");
	await expect(privateTranscript).not.toContainText("지원 채널 전용 메시지");
});

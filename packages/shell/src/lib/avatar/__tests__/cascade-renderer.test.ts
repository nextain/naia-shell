...
describe("CascadeAvatarRenderer.setVoice — PUT /voice 계약 (NVA 전환과 독립된 활성 음성)", () => {
	it("외부 프리셋 URL 을 **그대로 보내지 않고** 파일명으로 팔레트 URL 로 변환해 민다", async () => {
...
describe("useCascadeCharacter — 피커 폴더명 → 서버 bundle_id 해석 + /use_character", () => {
	const REGISTRY = [
		{ bundle_id: "Naia (기본 캐릭터)", name: "Naia (기본 캐릭터)" },
		{ bundle_id: "Minho", name: "Minho" },
		{ bundle_id: "Jina", name: "Jina" },
	];

	it("폴더명(jina) → 접두일치 bundle_id(Jina)로 POST /use_character", async () => {
		const fetchMock = vi
			.fn()
		...
		vi.stubGlobal("fetch", fetchMock);
		try {
			expect(await useCascadeCharacter("http://gpu:9449", "jina")).toBe(true);
	...
	it("naia → 'Naia (기본 캐릭터)' 접두일치 (한글·괄호 인코딩)", async () => {
		const fetchMock = vi
			.fn()
			.mockResolvedValueOnce({ ok: true, json: async () => REGISTRY })
			.mockResolvedValueOnce({ ok: true });
		vi.stubGlobal("fetch", fetchMock);
		try {
			expect(await useCascadeCharacter("http://gpu:9449", "naia")).toBe(true);
			expect(fetchMock.mock.calls[1][0]).toBe(
				`http://gpu:9449/use_character/${encodeURIComponent("Naia (기본 캐릭터)")}`,
			);
		} finally {
			vi.unstubAllGlobals();
		}
	...
	it("미등록 이름 = 전환 요청 안 보냄(false) — 서버 활성 캐릭터 유지", async () => {
		const fetchMock = vi
			.fn()
			.mockResolvedValueOnce({ ok: true, json: async () => REGISTRY });
		vi.stubGlobal("fetch", fetchMock);
...
describe("마스크 키잉 헬퍼 — NVA 플레이어(에디터 compose) 이식", () => {
	...
	it("sampleCornerKey: 최외곽 어두운 테두리(h264 경계 아티팩트)에 속지 않는다 — 인셋 샘플", () => {
		// 2026-07-16 실측(Jina idle): 배경 (254,240,213) 균일 + (2,2) 등 최외곽만 (239,225,199).
		// 구현이 최외곽에서 뽑으면 키가 어긋나 배경이 통째로 보존된다(회귀 고정).
		const w = 64;
		const d = solid(w, w, [254, 240, 213, 255]);
		for (let y = 0; y < w; y++)
			for (let x = 0; x < w; x++)
				if (x < 3 || y < 3 || x >= w - 3 || y >= w - 3)
					d.set([239, 225, 199, 255], (y * w + x) * 4);
		expect(sampleCornerKey(d, w, w)).toEqual([254, 240, 213]);
	});

	it("sampleCornerKey: 모서리가 투명(서버 알파 webm) → null (이중 키잉 금지)", () => {
		expect(sampleCornerKey(solid(64, 64, [0, 0, 0, 0]), 64, 64)).toBeNull();
	});

	it("sampleCornerKey: 샘플 지점 색 불일치(단색 배경 아님) → null (실사 등 키잉 금지)", () => {
		const d = solid(64, 64, [255, 242, 214, 255]);
		const ix = Math.max(4, Math.round(64 * 0.02)); // 구현과 동일 인셋 지점을 검정으로
		d.set([0, 0, 0, 255], (ix * 64 + ix) * 4);
		expect(sampleCornerKey(d, 64, 64)).toBeNull();
	});

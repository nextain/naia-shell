...
describe("UC1 통합 trace — 입력→송신→스트리밍→finish (풀 스택, mock Tauri)", () => {
  ...
  it("bridge 경유 완전 조립: ChatBridge.submit → ... → 렌더 → finish (실 trace 와 동일 경로)", async () => {
    const { deps, invokes, emit } = mockTauri();
    let seq = 0;
    const { router, bridge, sessions } = wireChatUC1({ live: deps, clientId: "shell", newRequestId: () => `t-${++seq}` });
    router.start();
    const rendered: ChatChunk[] = [];
    const { sent } = bridge.submit(
      { provider: { provider: "ollama", model: "gemma4" }, messages: [{ role: "user", content: "안녕" }] },
      (c) => rendered.push(c),
    );
    await sent; await Promise.resolve();
    const rid = JSON.parse(invokes[0]!.args["message"] as string).requestId;
    expect(rid).toBe("t-1"); // bridge 가 주입한 고유 id
    expect(sessions.ownerOf("t-1")).toBe("shell");
    emit(JSON.stringify({ type: "text", requestId: "t-1", text: "응답" }));
    emit(JSON.stringify({ type: "finish", requestId: "t-1" }));
    expect(rendered.map((c) => c.kind)).toEqual(["text", "finish"]);
    expect(sessions.ownerOf("t-1")).toBeUndefined();
  });

  it("fallback requestId(미주입) = 다중 wireChatUC1 인스턴스 간 고유(모듈전역 카운터, 충돌 없음)", async () => {
    const a = mockTauri(); const b = mockTauri();
    const wa = wireChatUC1({ live: a.deps }); const wb = wireChatUC1({ live: b.deps });
    wa.router.start(); wb.router.start();
    const ids = new Set<string>();
    for (let i = 0; i < 50; i++) {
      const ra = wa.bridge.submit({ provider: { provider: "o", model: "m" }, messages: [] }, () => {});
      const rb = wb.bridge.submit({ provider: { provider: "o", model: "m" }, messages: [] }, () => {});
      await ra.sent; await rb.sent;
    }
    for (const inv of [...a.invokes, ...b.invokes]) ids.add(JSON.parse(inv.args["message"] as string).requestId);
  ...
  it("UC-WIRE-V1 구조화 이벤트를 순서와 필드 손실 없이 전달한다", async () => {
    ...
    await sent; await Promise.resolve();
    emit(JSON.stringify({
      type: "grounding", requestId: "trace-1", status: "grounded",
      sources: [{ title: "수업 안내", sourceUris: ["kb://workshop"] }],
    }));
    emit(JSON.stringify({
      type: "artifact", requestId: "trace-1",
      artifact: { id: "a1", kind: "image", mimeType: "image/png", sizeBytes: 2, localRef: "img_1" },
    }));
    emit(JSON.stringify({
      type: "provider_session", requestId: "trace-1", sessionId: "s1",
      providerSessionRef: "opaque", state: "started",
    }));
    emit(JSON.stringify({
      type: "processing_disclosure", requestId: "trace-1", workload: "embedding",
      destination: "external_cloud", decision: "allowed",
      processingProfileRef: "profile-local-cloud-001",
    }));
    emit(JSON.stringify({
      type: "error", requestId: "trace-1", message: "invalid", code: "ATTACHMENT_INVALID_REF",
    }));
    expect(rendered.map((c) => c.kind)).toEqual(["grounding", "artifact", "providerSession", "processingDisclosure", "error"]);
    expect(rendered[0]).toMatchObject({ status: "grounded", sources: [{ sourceUris: ["kb://workshop"] }] });
    expect(rendered[1]).toMatchObject({ artifact: { localRef: "img_1" } });
    expect(rendered[3]).toMatchObject({ processingProfileRef: "profile-local-cloud-001", decision: "allowed" });
    expect(rendered[4]).toMatchObject({ code: "ATTACHMENT_INVALID_REF" });
  });
});

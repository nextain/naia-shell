...
describe("adapter 변환 (domain↔protocol↔wire, canon)", () => {
  ...
  it("Skills tab CLI selection reaches the canonical agent wire", async () => {
    const out = toAgentOutbound(req({ enabledClis: ["claude", "codex"] })) as Record<string, unknown>;
    expect(out["enabledClis"]).toEqual(["claude", "codex"]);
  });
...
describe("라이브 StdioTransportAdapter (주입형, Tauri 경계 — 앱 무접촉)", () => {
  ...
  it("enabled CLI selection is preserved through the live Tauri transport", async () => {
    const { deps, calls } = mockDeps();
    await makeLiveStdioTransport(deps).send(
      req({ enabledClis: ["claude", "codex"] }),
    );
    const message = JSON.parse(calls[0]!.args["message"] as string) as Record<string, unknown>;
    expect(message["enabledClis"]).toEqual(["claude", "codex"]);
  });
...
describe("ChatService + MessageRouter 계약", () => {
  ...
  it("unknown error code keeps the legacy message terminal", async () => {
    const { transport, sessions, chat } = wire();
    const got: ChatChunk[] = [];
    const { sent } = chat.startTurn(req(), (c) => got.push(c));
    await sent;
    transport.emit({ type: "error", requestId: "r1", message: "future boom", code: "FUTURE_CODE" });
    expect(got).toContainEqual({ kind: "error", message: "future boom" });
    expect(sessions.ownerOf("r1")).toBeUndefined();
  });

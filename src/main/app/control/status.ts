...
export class StatusReporter {
  ...
  report(): HonestStatusReport {
    ...
    try { system = this.interoceptive.systemStatus(); } catch (e) { probeErrors.push(`systemStatus: ${String(e)}`); }
    let degraded: DegradationSignal[] = [];
    try { degraded = this.interoceptive.degradations().filter(isDegraded); } catch (e) { probeErrors.push(`degradations: ${String(e)}`); }

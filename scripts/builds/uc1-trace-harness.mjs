...
// agent 가 startup 크래시/조기 종료(예: dist 미빌드 = Cannot find module)하면 timeout 대신 명확히 보고.
child.on("exit", (code, sig) => {
  if (agentExit === null) {
    agentExit = { code, sig };
    if (code !== 0 && code !== null) console.error(`[UC1-TRACE] ⚠ agent 프로세스 조기 종료 code=${code} sig=${sig} — agent 빌드/실행 확인(dist 미빌드면 'pnpm -C ../old-naia-os/agent i && pnpm -C ../old-naia-os/agent build').`);
  }
});

...
| S112 | Memory facts IPC latency smoke (memory_get_all_facts p50/p95) | spec 98 implemented; 10 sequential memory_get_all_facts calls, p50<50ms p95<150ms. Codex cross-review (2026-05-27) corrected the framing: the IPC reads alpha-memory.json via Rust fs+serde (memory.rs:122), NOT the naia-memory SQLite recall path — so this is a CI regression guard for the current Tauri+Rust+file-IO path, not a Tier-1 Surface benchmark. Latency reordered LAST in phase 2 (#332 §7). Deferred to Phase 4: direct memory_recall(query, topK, deep?) IPC (prereq for comparing against naia-memory v6.0's 9.74ms / 80ms), N=200 bulk-encode, Surface vs Deep separation. |
...
| S115 | Restart preserves login (single mode) | #337 Phase 10 — Node integration in `packages/runtime/src/auth/__tests__/restart-preserves-login.test.ts` (naia-agent repo). Verifies the encrypted `<ADK>/naia-settings/auth/dev.json.enc` blob decrypts after the in-process caches are reset (simulated cross-process boundary). Child-process spawn not viable due to pre-existing codegraph tsc errors blocking the runtime dist rebuild — see test header for rationale. |
| S116 | dev/prod mode swap — both sessions survive restart | #337 Phase 10 — same test file as S115. Verifies two distinct encrypted blobs (dev + prod), neither leaks naiaKey in plaintext, and auth_query succeeds in any mode-swap restart sequence. |
| S117 | Legacy migration happy path (secure-keys.dat → ADK file) | #337 Phase 10 — vitest at `shell/src/__tests__/legacy-migration-e2e.test.ts`. Uses real `secure-store.ts` against a stateful in-memory `@tauri-apps/plugin-store` mock; agent-ipc stubbed at module boundary. Verifies `kind:"migrated"`, IPC call args, and both `naiaKey` + `apiKey` (D3 garbage) slot purge. |
| S118 | Legacy migration ack failure — hard fail, slot preserved | #337 Phase 10 — same test file as S117. Verifies the hard-fail invariant (design §3): `{ok:false}` ack → `kind:"failed"`, listener fires, `naiaKey` slot left intact for retry, no fallback-to-shell-store path. |

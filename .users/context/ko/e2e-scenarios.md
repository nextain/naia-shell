...
| S112 | 메모리 레이턴시 버짓 — Tier-1 (Surface) 통합 경로 p50/p95 | spec 98 추가; memory_get_all_facts 10회 연속 호출로 p50<50ms / p95<150ms 검증 (naia-memory v6.0 라이브러리 벤치 9.74ms 대비 5배 완화 — IPC 프레이밍 + CI 환경 분산 흡수). 레이턴시는 #332 §7에 따라 phase 2 마지막으로 재배치 (검증 대상 경로가 안정화된 후 측정). Deferred: 직접 memory_recall(query, topK) IPC + N=200 일괄 인코드 + Deep tier 80ms 측정은 Phase 4로 이관. |

...
| S112 | 메모리 facts IPC 레이턴시 smoke (memory_get_all_facts p50/p95) | spec 98 추가; memory_get_all_facts 10회 연속 호출로 p50<50ms / p95<150ms 검증. Codex 교차 리뷰(2026-05-27) 정정: 본 IPC는 alpha-memory.json을 Rust fs+serde로 읽는 경로(memory.rs:122)이며 naia-memory SQLite recall 경로가 아님 — 따라서 본 spec은 Tier-1 Surface 벤치마크가 아닌 현재 Tauri+Rust+file-IO 경로의 CI 회귀 가드. 레이턴시는 #332 §7에 따라 phase 2 마지막으로 재배치. Deferred (Phase 4): 직접 memory_recall(query, topK, deep?) IPC (naia-memory v6.0 9.74ms / 80ms 비교의 전제조건), N=200 일괄 인코드, Surface vs Deep 분리. |
| S113 | 게이트웨이 임베딩 5xx 시 오프라인 ONNX 폴백 | spec 95 추가; 런타임 폴백 배선은 Phase 4로 지연 |
| S115 | 재시작 후에도 로그인 유지 (단일 모드) | #337 Phase 10 — `packages/runtime/src/auth/__tests__/restart-preserves-login.test.ts` (naia-agent 레포)의 Node integration. 인프로세스 캐시 리셋(크로스 프로세스 경계 시뮬레이션) 후에도 암호화된 `<ADK>/naia-settings/auth/dev.json.enc` 블롭이 복호화되는지 검증. 자식 프로세스 spawn은 기존 codegraph tsc 에러로 runtime dist 재빌드가 막혀 채택 불가 — 근거는 테스트 파일 헤더 참조. |
| S116 | dev/prod 모드 스왑 — 두 세션 모두 재시작 후 생존 | #337 Phase 10 — S115와 동일 파일. 별개의 두 암호화 블롭(dev + prod), 평문 naiaKey 미노출, 어떤 모드 스왑 시퀀스 후에도 auth_query 성공을 검증. |
| S117 | 레거시 마이그레이션 정상 경로 (secure-keys.dat → ADK 파일) | #337 Phase 10 — `shell/src/__tests__/legacy-migration-e2e.test.ts` vitest. 실제 `secure-store.ts`를 상태저장 인메모리 `@tauri-apps/plugin-store` mock 위에서 구동; agent-ipc는 모듈 경계에서 stub. `kind:"migrated"`, IPC 호출 인자, `naiaKey` + `apiKey`(D3 가비지) 슬롯 동시 정리 검증. |
| S118 | 레거시 마이그레이션 ack 실패 — 하드페일, 슬롯 보존 | #337 Phase 10 — S117과 동일 파일. design §3 하드페일 불변식 검증: `{ok:false}` ack → `kind:"failed"`, 리스너 발화, `naiaKey` 슬롯 보존(재시도 가능), shell-store 폴백 경로 없음. |

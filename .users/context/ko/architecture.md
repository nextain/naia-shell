...
- **Naia Gateway**가 "무슨 일이 있었는지" (세션 트랜스크립트 + 시맨틱 검색 인덱스)를 소유
...
## 브라우저 패널 — Chrome 임베딩 (#95, #164)

Chrome을 `PlatformWindowManager` trait 추상화로 Tauri에 네이티브 임베딩:
...
- **macOS:** 미구현 (`PlatformWindowManager` impl 추가 필요)

Chrome 탐색: Linux = `which` + Flatpak, Windows = `where.exe` + Program Files. Chrome 미설치 시 UI에 "Chrome 다운로드" 버튼 + 5초 자동감지 폴링.

AI 도구 12개: navigate, back, forward, reload, click, fill, scroll, press, snapshot, get_text, screenshot, eval.

---
name: healthcheck
description: Host security hardening and risk-tolerance configuration for Naia OS deployments. Use when a user asks for security audits, firewall/SSH/update hardening, risk posture, exposure review, naia cron scheduling for periodic checks, or version status checks on a machine running Naia (laptop, workstation, Pi, VPS).
---
...
Assess and harden the host running Naia, then align it to a user-defined risk tolerance without breaking access. Use naia security tooling as a first-class signal, but treat OS hardening as a separate, explicit set of steps.
...
- Never claim naia changes the host firewall, SSH, or OS updates; it does not.
...
5. naia gateway status and bind address.
...
9. OS automatic security updates status.
   Note: these are not blocking items, but are highly recommended, especially if naia can access sensitive data.
...
### 2) Run naia security audits (read-only)

As part of the default read-only checks, run naia security audit (naia-agent CLI pending). Only offer alternatives if the user requests them:

1. `naia security audit` (naia-agent CLI pending) (faster, non-probing)
2. `naia security audit --json` (naia-agent CLI pending) (structured output)

Offer to apply naia safe defaults (numbered):

1. `naia security audit --fix` (naia-agent CLI pending)

Be explicit that `--fix` only tightens naia defaults and file permissions. It does not change host firewall, SSH, or OS update policies.
...
### 3) Check naia version/update status (read-only)

As part of the default read-only checks, run `naia update status` (naia-agent CLI pending).
...
- Credential hygiene notes (location of naia creds, prefer disk encryption)
...
After naia install or first hardening pass, run at least one baseline audit and version check:

- `naia security audit` (naia-agent CLI pending)
- `naia security audit --deep` (naia-agent CLI pending)
- `naia update status` (naia-agent CLI pending)

Ongoing monitoring is recommended. Use the naia cron tool/CLI to schedule periodic audits (Gateway scheduler). Do not create scheduled tasks without explicit approval. Store outputs in a user-approved location and avoid secrets in logs.
...
1. “Do you want me to schedule periodic audits (e.g., daily/weekly) via `naia cron add`?”

If the user says yes, ask for:

- cadence (daily/weekly), preferred time window, and output location
- whether to also schedule `naia update status` (naia-agent CLI pending)
...
Before creating, `naia cron list` and match on exact `name`. If found, `naia cron edit <id> ...`.
If not found, `naia cron add --name <name> ...`.

Also offer a periodic version check so the user can decide when to update (numbered):

1. `naia update status` (naia-agent CLI pending) (preferred for source checkouts and channels)
2. `npm view naia version` (published npm version)

## naia command accuracy

Use only supported commands and flags:

- `naia security audit [--deep] [--fix] [--json]` (naia-agent CLI pending)
- `naia status` (naia-agent CLI pending) / `naia status --deep` (naia-agent CLI pending)
- `naia health --json` (naia-agent CLI pending)
- `naia update status` (naia-agent CLI pending)
- `naia cron add|list|runs|run`

Do not invent CLI flags or imply naia enforces host firewall/SSH policies.
...
Follow the durable-memory prompt format used by naia compaction:
